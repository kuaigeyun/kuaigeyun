...
class TenantConfig(BaseModel):
    ...
    class Meta:
        ...
        模型元数据
        """
        table = "core_tenant_configs"  # 表名必须包含模块前缀（core_）
        indexes = [
            ("tenant_id",),      # 租户 ID 索引
            ("config_key",),     # 配置键索引
            ("tenant_id", "config_key"),  # 租户 ID + 配置键联合索引（唯一性）
        ]
        unique_together = [("tenant_id", "config_key")]  # 同一租户下配置键唯一
