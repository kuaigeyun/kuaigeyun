...
var extractConnectionInformation = (store, extensionConnector, options) => {
  ...
  if (existingConnection) {
    return __spreadValues({ type: "tracked", store }, existingConnection);
  }
  const newConnection = {
    connection: extensionConnector.connect(options),
    stores: {}
  ...
  return __spreadValues({ type: "tracked", store }, newConnection);
};
var devtoolsImpl = (fn, devtoolsOptions = {}) => (set, get, api) => {
  const _a = devtoolsOptions, { enabled, anonymousActionType, store } = _a, options = __objRest(_a, ["enabled", "anonymousActionType", "store"]);
  ...
  const _b = extractConnectionInformation(store, extensionConnector, options), { connection } = _b, connectionInformation = __objRest(_b, ["connection"]);
  ...
  api.setState = (state, replace, nameOrAction) => {
    ...
    connection == null ? void 0 : connection.send(
      ...
      __spreadProps(__spreadValues({}, getTrackedConnectionState(options.name)), {
        [store]: api.getState()
      })
    );
    return r;
...
//# sourceMappingURL=chunk-LPELZQOY.js.map
