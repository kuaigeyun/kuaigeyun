...
function createRouter(init) {
  ...
  function handleFetcherAction(key, routeId, path, match, requestMatches, isFogOfWar, flushSync, submission) {
    return __async(this, null, function* () {
      interruptActiveLoads();
      ...
      if (isFogOfWar) {
        let discoverResult = yield discoverRoutes(requestMatches, path, fetchRequest.signal);
        if (discoverResult.type === "aborted") {
          return;
        } else if (discoverResult.type === "error") {
          let {
  ...
  function handleFetcherLoader(key, routeId, path, match, matches, isFogOfWar, flushSync, submission) {
    return __async(this, null, function* () {
      let existingFetcher = state.fetchers.get(key);
      ...
      if (isFogOfWar) {
        let discoverResult = yield discoverRoutes(matches, path, fetchRequest.signal);
        if (discoverResult.type === "aborted") {
          return;
        } else if (discoverResult.type === "error") {
          let {
  ...
  function discoverRoutes(matches, pathname, signal) {
    return __async(this, null, function* () {
      ...
      while (true) {
        ...
        let routesToUse = inFlightDataRoutes || dataRoutes;
        try {
          yield loadLazyRouteChildren(patchRoutesOnNavigationImpl, pathname, partialMatches, routesToUse, manifest, mapRouteProperties2, pendingPatchRoutes, signal);
        } catch (e) {
          return {
            type: "error",
            error: e,
...
function loadLazyRouteChildren(patchRoutesOnNavigationImpl, path, matches, routes, manifest, mapRouteProperties2, pendingRouteChildren, signal) {
  return __async(this, null, function* () {
...
function defaultDataStrategy(_ref6) {
  return __async(this, null, function* () {
    let {
      matches
    } = _ref6;
