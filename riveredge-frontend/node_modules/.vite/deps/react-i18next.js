...
var mergeProps = (source, target) => {
  const newTarget = __spreadValues({}, target);
...
var renderNodes = (children, targetString, i18n, i18nOptions, combinedTOpts, shouldUnescape) => {
  ...
  const opts = __spreadValues(__spreadValues({}, data), combinedTOpts);
  ...
  const pushTranslatedJSX = (child, inner, mem, i2, isVoid) => {
    if (child.dummy) {
      ...
      mem.push(...import_react.Children.map([child], (c2) => {
        const props = __spreadValues({}, c2.props);
        delete props.i18nIsDynamicList;
        return (0, import_react.createElement)(c2.type, __spreadProps(__spreadValues({}, props), {
          key: i2,
          ref: c2.ref
        }), isVoid ? null : inner);
      }));
...
function Trans(_ref) {
  ...
  const reactI18nextOptions = __spreadValues(__spreadValues({}, getDefaults()), i18n.options && i18n.options.react);
  ...
  if (i18n.options && i18n.options.interpolation && i18n.options.interpolation.defaultVariables) {
    values = values && Object.keys(values).length > 0 ? __spreadValues(__spreadValues({}, values), i18n.options.interpolation.defaultVariables) : __spreadValues({}, i18n.options.interpolation.defaultVariables);
  }
  const interpolationOverride = values || count !== void 0 || !children ? tOptions.interpolation : {
    interpolation: __spreadProps(__spreadValues({}, tOptions.interpolation), {
      prefix: "#$?",
      suffix: "?$#"
    })
  };
  const combinedTOpts = __spreadProps(__spreadValues(__spreadValues(__spreadProps(__spreadValues({}, tOptions), {
...
var composeInitialProps = (ForComponent) => (ctx) => __async(void 0, null, function* () {
  const componentsInitialProps = ForComponent.getInitialProps ? yield ForComponent.getInitialProps(ctx) : {};
  const i18nInitialProps = getInitialProps();
  return __spreadValues(__spreadValues({}, componentsInitialProps), i18nInitialProps);
});
var getInitialProps = () => {
  const i18n = getI18n();
...
var withTranslation = function(ns) {
  ...
  return function Extend(WrappedComponent) {
    function I18nextWithTranslation(_ref) {
      ...
      const [t2, i18n, ready] = useTranslation(ns, __spreadProps(__spreadValues({}, rest), {
        keyPrefix: options.keyPrefix
      }));
      const passDownProps = __spreadProps(__spreadValues({}, rest), {
        t: t2,
        i18n,
        tReady: ready
      });
...
var withSSR = () => function Extend(WrappedComponent) {
  function I18nextWithSSR(_ref) {
    ...
    return (0, import_react8.createElement)(WrappedComponent, __spreadValues({}, rest));
  }
