...
const withNullEntryPdfBytes = fs_1.default.readFileSync('assets/pdfs/with_null_parent_entry.pdf');
const pageUtils = () => {
    const context = index_1.PDFContext.create();
    const pageTree = (parent) => {
        const tree = index_1.PDFPageTree.withContext(context, parent);
        const ref = context.register(tree);
        return [ref, tree];
    };
    const pageLeaf = (parent) => {
        const leaf = index_1.PDFPageLeaf.withContextAndParent(context, parent);
        const ref = context.register(leaf);
        return [ref, leaf];
    };
    return { context, pageTree, pageLeaf };
};
const pushTreeNodes = (tree, ...nodes) => {
    nodes.forEach((n) => tree.pushTreeNode(n));
};
const pushLeafNodes = (tree, ...nodes) => {
...
describe(`PDFPageTree`, () => {
    it(`can be constructed directly from a Map and PDFContext`, () => {
        const context = index_1.PDFContext.create();
        const dict = new Map();
        const pageTree = index_1.PDFPageTree.fromMapWithContext(dict, context);
        expect(pageTree).toBeInstanceOf(index_1.PDFPageTree);
    ...
    it(`is constructed with the correct Type and entries`, () => {
        const context = index_1.PDFContext.create();
        const pageTree = index_1.PDFPageTree.withContext(context);
        expect(pageTree).toBeInstanceOf(index_1.PDFPageTree);
        expect(pageTree.get(index_1.PDFName.of('Type'))).toBe(index_1.PDFName.of('Pages'));
        expect(pageTree.get(index_1.PDFName.of('Kids'))).toBeInstanceOf(index_1.PDFArray);
        expect(pageTree.lookup(index_1.PDFName.of('Kids'), index_1.PDFArray).size()).toBe(0);
        expect(pageTree.get(index_1.PDFName.of('Count'))).toBeInstanceOf(index_1.PDFNumber);
        expect(pageTree.lookup(index_1.PDFName.of('Count'), index_1.PDFNumber).asNumber()).toBe(0);
        expect(pageTree.get(index_1.PDFName.of('Parent'))).toBeUndefined();
    });
    it(`returns its Parent, Kids, and Count entry values when they are references`, () => {
        ...
        const pageTree = index_1.PDFPageTree.withContext(context, parentRef);
        pageTree.set(index_1.PDFName.of('Kids'), kidsRef);
        pageTree.set(index_1.PDFName.of('Count'), countRef);
    ...
    it(`returns its Parent, Kids, and Count entry values when they are direct objects`, () => {
        ...
        const count = context.obj(0);
        const pageTree = index_1.PDFPageTree.withContext(context);
        pageTree.set(index_1.PDFName.of('Kids'), kids);
        pageTree.set(index_1.PDFName.of('Count'), count);
    ...
    describe(`leaf node insertion`, () => {
        it(`can insert leaf nodes into the middle of the second layer of a page tree`, () => {
            ...
            const newLeaf = index_1.PDFPageLeaf.withContextAndParent(context, lvl2Tree1Ref);
        ...
        it(`can insert leaf nodes at the end of a page tree`, () => {
            ...
            const newLeaf = index_1.PDFPageLeaf.withContextAndParent(context, lvl1Tree1Ref);
            const newLeafRef = context.register(newLeaf);
    ...
    describe(`leaf node removal`, () => {
        ...
        it(`throws an error when removing past the end of a tree`, () => {
            ...
            expect(() => buildTree().removeLeafNode(3)).toThrow('Invalid targetIndex specified: targetIndex=3 must be less than Count=3');
            expect(() => buildTree().removeLeafNode(4)).toThrow('Invalid targetIndex specified: targetIndex=4 must be less than Count=3');
        });
        it(`does not throw an error when at the end of a hierarchical tree`, () => {
            ...
            expect(() => buildTree().removeLeafNode(8)).toThrow('Invalid targetIndex specified: targetIndex=8 must be less than Count=8');
            expect(() => buildTree().removeLeafNode(9)).toThrow('Invalid targetIndex specified: targetIndex=9 must be less than Count=8');
        });
        it(`throws an error when the page tree has no kids`, () => {
            ...
            expect(() => buildTree().removeLeafNode(0)).toThrow('Invalid targetIndex specified: targetIndex=0 must be less than Count=0');
            expect(() => buildTree().removeLeafNode(1)).toThrow('Invalid targetIndex specified: targetIndex=1 must be less than Count=0');
        });
        it(`throws an error when the page tree is invalid`, () => {
            ...
            tree.set(index_1.PDFName.of('Count'), index_1.PDFNumber.of(21));
            expect(() => tree.removeLeafNode(0)).toThrow(`Failed to removeLeafNode at targetIndex=0 due to corrupt page tree: It is likely that one or more 'Count' entries are invalid`);
            expect(() => tree.removeLeafNode(1)).toThrow(`Failed to removeLeafNode at targetIndex=1 due to corrupt page tree: It is likely that one or more 'Count' entries are invalid`);
        });
    ...
    it(`can be ascended when a "/Parent null" entry exists on a node`, async () => {
        const pdfDoc = await index_1.PDFDocument.load(withNullEntryPdfBytes);
        const pages = pdfDoc.getPages();
        const parent = pages[0].node.Parent();
        expect(parent).toBeInstanceOf(index_1.PDFPageTree);
        let ascentions = 0;
        const handleAscend = () => {
            ascentions += 1;
        };
...
//# sourceMappingURL=PDFPageTree.spec.js.map