import fs from 'fs';
import { CharCodes, mergeIntoTypedArray, PDFDict, PDFHeader, PDFInvalidObject, PDFPageLeaf, PDFParser, PDFRef, PDFString, ReparseError, typedArrayFor, } from '../../../src/index';
describe(`PDFParser`, () => {
    ...
    it(`handles invalid binary comments after header`, async () => {
        const input = mergeIntoTypedArray('%PDF-1.7\n', new Uint8Array([128, 1, 2, 3, 4, 5, 129, 130, 131, CharCodes.Newline]), '1 0 obj\n', ' . (foobar)\n', 'endobj');
        const parser = PDFParser.forBytesWithOptions(typedArrayFor(input));
        const context = await parser.parseDocument();
  ...
  `;
    ...
    it(`prevents double parsing`, async () => {
        ...
        await expect(parser.parseDocument()).rejects.toThrow(new ReparseError('PDFParser', 'parseDocument'));
    });
