import fs from 'fs';
import { PDFArray, PDFBool, PDFContext, PDFDict, PDFHexString, PDFName, PDFNull, PDFNumber, PDFObjectStreamParser, PDFRawStream, PDFRef, PDFString, ReparseError, } from '../../../src/index';
const readData = (file) => new Uint8Array(fs.readFileSync(`${__dirname}/data/${file}`));
describe(`PDFObjectStreamParser`, () => {
    ...
    it(`throws an error for invalid Filters`, () => {
        ...
        const contents = readData('object-stream4');
        const stream = PDFRawStream.of(dict, contents);
        expect(() => PDFObjectStreamParser.forStream(stream).parseIntoContext()).toThrow();
    });
    ...
    it(`prevents reparsing`, async () => {
        ...
        await expect(parser.parseIntoContext()).rejects.toThrow(new ReparseError('PDFObjectStreamParser', 'parseIntoContext'));
    });
});
