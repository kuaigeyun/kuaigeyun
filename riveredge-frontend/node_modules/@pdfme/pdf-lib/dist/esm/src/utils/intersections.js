...
import { distance, isColinear, isEqual, norm, orthogonal, times, unitVector, vector, rotate, } from './maths';
...
export function intersectionLine(A, B) {
...
export function intersectionCircleLine(A, B) {
    const rA = A.ray();
    const O = A.center();
...
export function intersectionCircle(A, B) {
    const oA = A.center();
    const oB = B.center();
    const rA = A.ray();
    const rB = B.ray();
...
export function getIntersections(elements) {
    const checked = [];
    const inters = [];
