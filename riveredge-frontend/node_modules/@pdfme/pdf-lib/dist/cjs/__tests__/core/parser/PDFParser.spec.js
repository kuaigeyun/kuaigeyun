...
describe(`PDFParser`, () => {
    ...
    it(`throws an error when the PDF is missing a header`, async () => {
      ...
      endobj
    `;
        const parser = index_1.PDFParser.forBytesWithOptions((0, index_1.typedArrayFor)(input));
    ...
    `;
        ...
        expect(context.lookup(index_1.PDFRef.of(1))).toBeInstanceOf(index_1.PDFString);
    });
    it(`handles invalid binary comments after header`, async () => {
        const input = (0, index_1.mergeIntoTypedArray)('%PDF-1.7\n', new Uint8Array([128, 1, 2, 3, 4, 5, 129, 130, 131, index_1.CharCodes.Newline]), '1 0 obj\n', ' . (foobar)\n', 'endobj');
        const parser = index_1.PDFParser.forBytesWithOptions((0, index_1.typedArrayFor)(input));
        const context = await parser.parseDocument();
    ...
    it(`handles invalid binary comments with missing newline after header`, async () => {
        const input = (0, index_1.mergeIntoTypedArray)('%PDF-1.7\n', new Uint8Array([142, 1, 2, 3, 4, 5, 129, 130, 131]), '1 0 obj\n', ' . (foobar)\n', 'endobj');
        const parser = index_1.PDFParser.forBytesWithOptions((0, index_1.typedArrayFor)(input));
        const context = await parser.parseDocument();
        expect(context.enumerateIndirectObjects().length).toBe(1);
    });
  ...
  `;
        ...
        const context = await parser.parseDocument();
        expect(context.enumerateIndirectObjects().length).toBe(1);
        const object = context.lookup(index_1.PDFRef.of(22));
        expect(object).toBeInstanceOf(index_1.PDFInvalidObject);
    });
    ...
    22 0 obj <</Type/Outlines/First ## 0 R/Last ** 0 R/Count 2>> endobj
  `;
        const parser = index_1.PDFParser.forBytesWithOptions((0, index_1.typedArrayFor)(input), 100, true);
  ...
  `;
        ...
        expect(context.enumerateIndirectObjects().length).toBe(1);
        const object = context.lookup(index_1.PDFRef.of(22));
        expect(object).toBeInstanceOf(index_1.PDFString);
    });
    it(`can parse PDF files with comments and stuff preceding the header`, async () => {
        const pdfBytes = fs_1.default.readFileSync('./assets/pdfs/pdf20examples/PDF 2.0 with offset start.pdf');
        const parser = index_1.PDFParser.forBytesWithOptions(pdfBytes);
        const context = await parser.parseDocument();
        expect(context.header).toBeInstanceOf(index_1.PDFHeader);
    ...
    it(`can parse PDF files with comments stuff following the header`, async () => {
        const pdfBytes = fs_1.default.readFileSync('./assets/pdfs/stuff_following_header.pdf');
        const parser = index_1.PDFParser.forBytesWithOptions(pdfBytes);
        const context = await parser.parseDocument();
        expect(context.header).toBeInstanceOf(index_1.PDFHeader);
    ...
    it(`can parse PDF files with missing xref table, trailer dict, and trailer`, async () => {
        const pdfBytes = fs_1.default.readFileSync('./assets/pdfs/missing_xref_trailer_dict.pdf');
        const parser = index_1.PDFParser.forBytesWithOptions(pdfBytes);
        const context = await parser.parseDocument();
        expect(context.header).toBeInstanceOf(index_1.PDFHeader);
        expect(context.header.toString()).toEqual('%PDF-2.0\n%');
    ...
    it(`can parse PDF files without object streams or update sections`, async () => {
        const pdfBytes = fs_1.default.readFileSync('./assets/pdfs/normal.pdf');
        const parser = index_1.PDFParser.forBytesWithOptions(pdfBytes);
        const context = await parser.parseDocument();
        expect(context.header).toBeInstanceOf(index_1.PDFHeader);
    ...
    it(`can parse PDF files with update sections`, async () => {
        const pdfBytes = fs_1.default.readFileSync('./assets/pdfs/with_update_sections.pdf');
        const parser = index_1.PDFParser.forBytesWithOptions(pdfBytes);
        const context = await parser.parseDocument();
        expect(context.header).toBeInstanceOf(index_1.PDFHeader);
    ...
    it(`can parse PDF files with comments`, async () => {
        const pdfBytes = fs_1.default.readFileSync('./assets/pdfs/with_comments.pdf');
        const parser = index_1.PDFParser.forBytesWithOptions(pdfBytes);
        const context = await parser.parseDocument();
        expect(context.header).toBeInstanceOf(index_1.PDFHeader);
        expect(context.header.toString()).toEqual('%PDF-1.7\n%');
    ...
    it(`prevents double parsing`, async () => {
        ...
        await expect(parser.parseDocument()).rejects.toThrow(new index_1.ReparseError('PDFParser', 'parseDocument'));
    });
    it(`can parse PDF files with binary jibberish between indirect objects`, async () => {
        const pdfBytes = fs_1.default.readFileSync('./assets/pdfs/giraffe.pdf');
        const parser = index_1.PDFParser.forBytesWithOptions(pdfBytes);
        const context = await parser.parseDocument();
        expect(context.header).toBeInstanceOf(index_1.PDFHeader);
    ...
    it(`can fix incorrect values for /Root`, async () => {
        const pdfBytes = fs_1.default.readFileSync('./assets/pdfs/invalid_root_ref.pdf');
        const parser = index_1.PDFParser.forBytesWithOptions(pdfBytes);
        const context = await parser.parseDocument();
        expect(context.header).toBeInstanceOf(index_1.PDFHeader);
        expect(context.header.toString()).toEqual('%PDF-1.5\n%');
        expect(context.trailerInfo.Root).toBe(index_1.PDFRef.of(2, 0));
    ...
    it(`can parse files containing indirect objects missing their 'endobj' keyword`, async () => {
        const pdfBytes = fs_1.default.readFileSync('./assets/pdfs/missing_endobj_keyword.pdf');
        const parser = index_1.PDFParser.forBytesWithOptions(pdfBytes);
        const context = await parser.parseDocument();
        expect(context.header).toBeInstanceOf(index_1.PDFHeader);
        expect(context.header.toString()).toEqual('%PDF-1.3\n%');
    ...
    it(`can parse files with containing large arrays with most 'null' values`, async () => {
        const pdfBytes = fs_1.default.readFileSync('./assets/pdfs/bixby_guide.pdf');
        const parser = index_1.PDFParser.forBytesWithOptions(pdfBytes);
        const context = await parser.parseDocument();
        expect(context.header).toBeInstanceOf(index_1.PDFHeader);
        expect(context.header.toString()).toEqual('%PDF-1.4\n%');
        ...
        expect(objects.filter(([_ref, obj]) => obj instanceof index_1.PDFPageLeaf).length).toBe(176);
    });
    it(`can parse files with invalid stream EOLs: "stream \r\n`, async () => {
        const pdfBytes = fs_1.default.readFileSync('./assets/pdfs/with_invalid_stream_EOL.pdf');
        const parser = index_1.PDFParser.forBytesWithOptions(pdfBytes);
        const context = await parser.parseDocument();
        expect(context.header).toBeInstanceOf(index_1.PDFHeader);
        expect(context.header.toString()).toEqual('%PDF-1.3\n%');
        ...
        expect(objects.filter(([_ref, obj]) => obj instanceof index_1.PDFPageLeaf).length).toBe(2);
    });
  ...
  `;
        ...
        const object22 = context.lookup(index_1.PDFRef.of(22));
        expect(object22).toBeInstanceOf(index_1.PDFString);
        const object28 = context.lookup(index_1.PDFRef.of(28));
        expect(object28).toBeInstanceOf(index_1.PDFDict);
    });
  ...
  `;
        ...
        const context = await parser.parseDocument();
        expect(context.enumerateIndirectObjects().length).toBe(2);
        const object1 = context.lookup(index_1.PDFRef.of(1));
        expect(object1).toBeInstanceOf(index_1.PDFString);
        const object0 = context.lookup(index_1.PDFRef.of(0));
        expect(object0).toBe(undefined);
        const object2 = context.lookup(index_1.PDFRef.of(2));
        expect(object2).toBeInstanceOf(index_1.PDFString);
    });
});
