...
// import log from './log'
const deepEqual_js_1 = __importDefault(require("./deepEqual.js"));
const deepClone_js_1 = __importDefault(require("./deepClone.js"));
const cursor_js_1 = require("./cursor.js");
const types_js_1 = require("./types.js");
const RELEASE_MODE = process.env.JSON1_RELEASE_MODE;
const log = RELEASE_MODE ? (() => { }) : require('./log').default;
...
function assert(pred, msg) {
    if (!RELEASE_MODE)
        require('assert')(pred, msg); // For nicer error messages in node.
    else if (!pred)
        throw new Error(msg);
}
let debugMode = false;
exports.type = {
    ...
    isNoop(op) { return op == null; },
    setDebug(val) { debugMode = val; log.quiet = !val; },
    registerSubtype,
    checkValidOp,
    normalize,
    apply,
    ...
    typeAllowingConflictsPred: (allowConflict) => (Object.assign(Object.assign({}, exports.type), { transform(op1, op2, side) {
...
const shallowClone = (obj) => (Array.isArray(obj)
    ? obj.slice()
...
exports.removeOp = (path, value = true) => (cursor_js_1.writeCursor()
    .writeAtPath(path, 'r', value)
    .get());
exports.moveOp = (from, to) => (cursor_js_1.writeCursor()
    .writeMove(from, to)
    .get());
exports.insertOp = (path, value) => (cursor_js_1.writeCursor()
...
exports.editOp = (path, type, subOp, preserveNoop = false) => (cursor_js_1.writeCursor()
...
const replaceChild = (obj, k, v) => {
    ...
    obj[k] = v; // Could add an assert here...
    return obj;
};
const isValidKey = (container, key) => (container == null
    ? false
    : typeof key === 'number'
        ? Array.isArray(container)
        : typeof container === 'object');
const maybeGetChild = (container, key) => (isValidKey(container, key) ? container[key] : undefined);
const subtypes = {};
function registerSubtype(subtype) {
    let _subtype = subtype.type ? subtype.type : subtype;
    if (_subtype.name)
        subtypes[_subtype.name] = _subtype;
    if (_subtype.uri)
        subtypes[_subtype.uri] = _subtype;
}
...
const writeEdit = (w, typeOrName, edit, preserveNoop = false) => {
    ...
    if (!preserveNoop && type.isNoop && type.isNoop(edit))
        return;
    if (name === 'number') {
        w.write('ena', edit);
    }
...
function checkValidOp(op) {
    ...
    if (op === null)
        return; // Empty operation. undefined is not a valid operation and should crash.
    ...
    const checkComponent = (e) => {
        ...
        for (let k in e) {
            ...
            assert(k === 'p' || k === 'r' || k === 'd' || k === 'i'
                || k === 'e' || k === 'es' || k === 'ena' || k === 'et', "Invalid component item '" + k + "'");
            if (k === 'p') {
        ...
        for (let i = 0; i < descent.length; i++) {
            ...
            if (Array.isArray(d)) {
                ...
                if (numDescents) {
                    ...
                    const t2 = typeof key;
                    if (t1 === t2)
                        assert(lastKey < key, 'descent keys are not in order');
                    else
                        assert(t1 === 'number' && t2 === 'string');
                }
                lastKey = key;
                numDescents++;
                last = DESCENT;
                ...
                assert(last !== DESCENT);
                checkScalar(d);
                assert(cursor_js_1.isValidPathItem(d), 'Invalid path key'); // They're trying to use __proto__ or something.
                last = SCALAR;
...
function normalize(op) {
    ...
    const getSlot = (inSlot) => {
        if (slotMap[inSlot] == null)
            slotMap[inSlot] = nextSlot++;
        ...
        for (--i; i >= 0; i--) {
            ...
            if (typeof d !== 'object') {
                ...
                subDoc = (subDoc === expectedChild)
                    ? container
                    : (subDoc === undefined)
                        ? removeChild(container, d)
                        : replaceChild(container, d, subDoc);
            }
            else if (hasPick(d)) {
    ...
    function drop(root, descent) {
        ...
        for (; i < descent.length; i++) {
            ...
            if (Array.isArray(d)) {
                ...
                if (d.d != null) { // Drop
                    mut();
                    subDoc = insertChildMut(container, key, held[d.d]);
                }
                ...
                if (t) { // Edit. Ok because its illegal to drop inside mixed region
                    mut();
                    subDoc = container[key] = t.apply(subDoc, getEdit(d));
                }
...
const incPrefix = () => { if (!RELEASE_MODE)
    log.prefix++; };
const decPrefix = () => { if (!RELEASE_MODE)
    log.prefix--; };
/**
...
function transformPosition(path, op) {
    ...
    let removed = false;
    let pickedAtSlot;
    let pickIndex; // index in path where pick happened
    const advStack = [];
    for (let i = 0;; i++) {
        const k = path[i];
        ...
        if (i >= path.length)
            break;
        let pickOffset = 0;
        const pickAdv = cursor_js_1.advancer(r, undefined, (k, c) => {
            if (hasPick(c)) {
                pickOffset++;
    ...
    decPrefix();
    log('after pick phase. Remove', removed, 'pi', pickIndex, 'pas', pickedAtSlot, 'po', path);
    ...
    const handleDrop = () => {
        ...
        if (pickedAtSlot != null) {
            ...
            i = rPath.length;
            if (!RELEASE_MODE)
                log('path', path, 'pi', pickIndex, 'r', rPath);
            path = rPath.concat(path.slice(pickIndex));
        }
        if (!RELEASE_MODE)
            log('handleDrop at path', path, 'read cursor', r.getPath());
        ...
        for (; i < path.length; i++) {
            ...
            let dropOffset = 0;
            const dropAdv = cursor_js_1.advancer(r, (k, c) => (hasDrop(c) ? ~(k - dropOffset) : k - dropOffset), (k, c) => {
                if (hasDrop(c))
                    dropOffset++;
            });
...
function compose(op1, op2) {
    ...
    if (op2 == null)
        return op1;
    let nextSlot = 0;
    const r1 = cursor_js_1.readCursor(op1), r2 = cursor_js_1.readCursor(op2);
    const w = cursor_js_1.writeCursor();
    ...
    function xfBoundary(r1Pick, r1Drop, r2Pick, r2Drop, litIn, rmParent, wd, wp) {
        ...
        if (!RELEASE_MODE)
            log('xfBoundary', litIn, !!r1Pick && r1Pick.getPath(), !!r1Drop && r1Drop.getPath(), !!r2Pick && r2Pick.getPath(), !!r2Drop && r2Drop.getPath(), 'lit:', litIn, 'rmParent:', rmParent);
        incPrefix();
        ...
        if (pick2Slot != null) {
            ...
            wd = heldDropWrites[pick2Slot] = new cursor_js_1.WriteCursor();
            // rmParent = null
        }
        ...
        if (drop1Slot != null) { // Drop here.
            ...
            // Writes here need to be transformed to op2's drop location.
            wp = heldPickWrites[drop1Slot] = new cursor_js_1.WriteCursor();
            if (!rmChildren) {
        ...
        const insComponent = (pick2Slot == null
            ...
            : litOut !== undefined) ? wd.getComponent() : null;
        if (insComponent)
            log('insComponent', (insHere && !rmParent && !rmHere && pick2Slot == null), (pick2Slot != null && litOut !== undefined));
        else
            log('no insComponent - this insert will be discarded', (insHere && !rmParent && !rmHere && pick2Slot == null), (pick2Slot != null && litOut !== undefined));
        if (pick2Slot != null) {
            // ....
        ...
        if (type1 || type2)
            log('types', type1 && type1.name, type2 && type2.name);
        if (type1 && type2) {
            assert(type1 === type2);
            // Compose.
        ...
        let litOff = 0;
        const p2DropAdv = cursor_js_1.advancer(r2Drop, (k, c) => hasDrop(c) ? (p2DropOff - k - 1) : k - p2DropOff, (k, c) => { if (hasDrop(c))
            p2DropOff++; });
        const p1PickAdv = cursor_js_1.advancer(r1Pick, (k, c) => hasPick(c) ? (p1PickOff - k - 1) : k - p1PickOff, (k, c) => { if (hasPick(c))
            p1PickOff++; });
        cursor_js_1.eachChildOf(r1Drop, r2Pick, (inKey, _p1Drop, _p2Pick) => {
            ...
            if (typeof inKey === 'number') {
                ...
                if (hasDrop(getComponent(_p2Drop)))
                    _p1Pick = null; // But still advance!
                p1PickKey = p1Mid + p1PickOff;
                // If the literal is an array, we're modifying that array as we go.
                litKey = inKey + litOff;
                log('p1PickOff', p1PickOff, 'p1DropOff', p1DropOff, 'p2PickOff', p2PickOff, 'p2DropOff', p2DropOff, 'litOff', litOff);
            ...
            const _litResult = xfBoundary(_p1Pick, _p1Drop, _p2Pick, _p2Drop, _lit, rmChildren, wd, wp);
...
function invert(op) {
    ...
    if (op == null)
        return null;
    const r = new cursor_js_1.ReadCursor(op);
    const w = new cursor_js_1.WriteCursor();
    ...
    function invertSimple(r, w, subDoc) {
        if (!RELEASE_MODE)
            log('invertSimple', r.getPath(), subDoc);
        incPrefix();
        const c = r.getComponent();
    ...
    if (editsToTransform) {
        ...
        function transformEdits(rPick, rDrop, w) {
            if (!RELEASE_MODE)
                log('invXE', rPick === null || rPick === void 0 ? void 0 : rPick.getPath(), rDrop.getPath(), w.getPath());
            incPrefix();
            ...
            if (cd) {
                ...
                if (dropSlot != null) {
                    ...
                    w = heldWrites[dropSlot] = cursor_js_1.writeCursor();
                }
                if (editsToTransform.has(cd)) {
                    const t = getEditType(cd);
                    // TODO: Add support for types which only have invertWithDoc.
                    if (!t.invert)
                        throw Error(`Cannot invert subtype ${t.name}`);
                    if (!RELEASE_MODE)
                        log('inverting subtype', t.name, getEdit(cd));
                    writeEdit(w, t, t.invert(getEdit(cd)));
                }
            }
            // And recurse.
            let pickOff = 0, dropOff = 0;
            const ap = cursor_js_1.advancer(rPick, (k, c) => hasPick(c) ? (pickOff - k - 1) : k - pickOff, (k, c) => { if (hasPick(c))
                pickOff++; });
        ...
        if (heldWrites.length) {
            if (!RELEASE_MODE)
                log('Merging held writes', heldWrites.map(w => w.get()));
            w.reset();
            // Merge held writes
            r.traverse(w, (c, w) => {
                log('traverse', c);
...
const anyComponent = (op, fn) => (op.some(c => (typeof c === 'object'
...
function makeInvertible(op, doc) {
    ...
    if (op == null || !anyComponent(op, c => {
        var _a;
        return (c.r !== undefined || ((_a = getEditType(c)) === null || _a === void 0 ? void 0 : _a.makeInvertible) != null);
    }))
        return op;
    const r = new cursor_js_1.ReadCursor(op);
    const w = new cursor_js_1.WriteCursor();
    ...
    const heldPick = [];
    const heldDoc = [];
    const traversePick = (r, w, subDoc) => {
        if (!RELEASE_MODE)
            log('traversePick', r.getPath(), subDoc);
        incPrefix();
        const c = r.getComponent();
        let modified = false;
        if (c) {
        ...
        for (const key of r) {
            ...
            if (childIn !== childOut) {
                ...
                if (childOut === undefined) {
                    subDoc = removeChild(subDoc, keyRaw);
                    if (typeof key === 'number')
                        listOff++;
                }
                else
                    subDoc[keyRaw] = childOut;
                log('subDoc ->', subDoc);
            }
            w.ascend();
        }
        if (c) {
            if (c.r !== undefined) {
                ...
                w.write('r', deepClone_js_1.default(subDoc));
                subDoc = undefined;
            }
            else if (c.p != null) {
    ...
    if (hasEdits) {
        w.reset();
        function traverseDrop(rPick, rDrop, w, subDoc, isLiteral) {
            if (!RELEASE_MODE)
                log('traverseDrop', rPick === null || rPick === void 0 ? void 0 : rPick.getPath(), rDrop.getPath(), w.getPath(), subDoc);
            incPrefix();
...
function tryTransform(op1, op2, direction) {
    ...
    let conflict = null;
    const heldOp1PickByOp1 = [];
    const heldOp1DropByOp1 = [];
    const heldOp2PickByOp2 = []; // Used in scanOp2Drop
    const heldOp2DropByOp2 = []; // Used in writeOp1Pick
    const heldOp1PickByOp2 = [];
    const heldOp2PickByOp1 = []; // Used in writeOp1Drop
    const heldOp2DropByOp1 = []; // Used in writeOp1Drop
    ...
    const r1 = cursor_js_1.readCursor(op1);
    const r2 = cursor_js_1.readCursor(op2);
    const w = cursor_js_1.writeCursor(); // Output.
    // ---------------------------------------------------------------------
    ...
    function scanOp2Pick(r2Pick, r1Pick = null, removed1) {
        if (!RELEASE_MODE)
    ...
    function scanOp2Drop(r1Pick, r2Pick, r2Drop, pickSlot1, removed1) {
        if (!RELEASE_MODE)
            log('scanOp2Drop', 'r1Pick', r1Pick && r1Pick.getPath(), r2Pick && r2Pick.getPath(), 'r2Drop', r2Drop.getPath(), 'pickSlot1', pickSlot1, 'removed:', removed1 && removed1.getPath());
        incPrefix();
        ...
        if (c2d) {
            if (((slot2 = c2d.d) != null)) {
                ...
                if (pickSlot1 != null) {
                    if (op1PicksOp2DropSlots[pickSlot1] == null)
        ...
        if (t2 && removed1) {
            ...
            if (conflict == null)
                conflict = {
                    type: types_js_1.ConflictType.RM_UNEXPECTED_CONTENT,
                    op1: exports.removeOp(removed1.getPath()),
                    op2: exports.editOp(r2Drop.getPath(), t2, getEdit(c2d), true),
                };
        }
        // And recurse...
        let p2PickOff = 0, p2DropOff = 0;
        const ap2 = cursor_js_1.advancer(r2Pick, (k, c) => hasPick(c) ? (p2PickOff - k - 1) : k - p2PickOff, (k, c) => { if (hasPick(c))
            p2PickOff++; });
    ...
    let cancelledRemoves = null; // Lazy initialized set of components.
    ...
    function writeOp1Pick(r1Pick, r2Pick, r2Drop, w, removed2) {
        if (!RELEASE_MODE)
            log('writeOp1Pick', 'r1Pick', r1Pick.getPath(), 'r2Pick', r2Pick && r2Pick.getPath(), 'r2Drop', r2Drop && r2Drop.getPath(), 'w', w.getPath(), 'removed2', removed2 && removed2.getPath());
        incPrefix();
        ...
        if (hasPick(c2p)) {
            ...
            if (slot2 != null) {
                ...
                if (!RELEASE_MODE)
                    log('teleporting via op2 slot2', slot2, 'to', heldOp2DropByOp2[slot2].getPath());
                ...
                w = heldPickWrites[slot2] = cursor_js_1.writeCursor();
        ...
        if (c1) {
            ...
            if (slot1 != null) {
                ...
                if (r2Drop)
                    heldOp2DropByOp1[slot1] = r2Drop.clone();
            }
                ...
                if (removed2 || iAmMoved) {
                    ...
                    if (cancelledRemoves == null)
                        cancelledRemoves = new Set;
        ...
        const ap2Pick = cursor_js_1.advancer(r2Pick, undefined, (k, c) => {
            if (hasPick(c)) {
                p2PickOff++;
        ...
        // ap2Pick._name = '2pick'
        const ap2Drop = cursor_js_1.advancer(r2Drop, (k, c) => (
        // Sneaking a second bit in here via 2s compliment.
        hasDrop(c) ? ~(k - p2DropOff) : k - p2DropOff), (k, c) => {
            if (hasDrop(c)) {
                p2DropOff++;
    ...
    function writeOp1Drop(p1Pick, p1Drop, p2Pick, p2Drop, w, removed2 /*, insidePick2*/) {
        assert(p1Drop);
        const c1d = p1Drop.getComponent();
        if (!RELEASE_MODE)
            log('drop', p1Pick && p1Pick.getPath(), p1Drop.getPath(), p2Pick && p2Pick.getPath(), p2Drop && p2Drop.getPath(), 'c:', c1d, 'r:', !!removed2 /*, 'ip2', insidePick2*/);
        incPrefix();
        ...
        if (hasDrop(c1d)) {
            ...
            if (slot1 != null)
                heldOp1DropByOp1[slot1] = p1Drop.clone();
            ...
            if (c1d.i !== undefined || ((slot1 != null) && pc)) {
                ...
                if (c2d && (c2d.i !== undefined || ((slot2 = c2d.d) != null && !cancelledOp2[slot2]))) {
                    identical = slot2 != null
                        ...
                        : deepEqual_js_1.default(c2d.i, c1d.i);
                ...
                if (!identical) {
                    if (removed2) {
                        ...
                        if (slot1 != null) {
                            ...
                            log('copying insert');
                            w.write('i', deepClone_js_1.default(c1d.i)); // TODO: Only clone based on opts.consume.
                        }
                        droppedHere = true;
                    }
                }
            ...
            if (slot1 != null) {
                ...
                if (!RELEASE_MODE)
                    log('p1Pick', p1Pick && p1Pick.getPath(), 'p2Pick', p2Pick && p2Pick.getPath(), 'p2Drop', p2Drop && p2Drop.getPath());
            }
                ...
                p1Pick = p2Pick = null;
                if (!identical)
                    p2Drop = null; // If identical, we'll merge the p2 child drops.
        ...
        if (hasPick(c2p)) {
            ...
            if ((c2p.r !== undefined && (!c1p || c1p.r === undefined)) || cancelledOp2[slot2]) {
                ...
                if (side === RIGHT || op1PickAtOp2Pick[slot2] == null) {
                    ...
                    if (!(w = heldDropWrites[slot2]))
                        w = heldDropWrites[slot2] = cursor_js_1.writeCursor();
                    w.reset();
                    removed2 = null;
                }
            }
        }
        else if (!hasDrop(c1d) && hasDrop(c2d))
            p2Drop = null;
        if (!RELEASE_MODE)
            uniqDesc('op1drop op2Drop', p2Drop);
        ...
        if (t1) {
            ...
            if (!removed2) {
                ...
                if (conflict == null)
                    conflict = {
                        type: types_js_1.ConflictType.RM_UNEXPECTED_CONTENT,
                        op1: exports.editOp(p1Drop.getPath(), t1, e1, true),
                        op2: exports.removeOp(removed2.getPath()),
                    };
            }
        }
        ...
        const ap2p = cursor_js_1.advancer(p2Pick, undefined, (k, c) => {
        ...
        for (const key of p1Drop) {
            ...
            if (typeof key === 'number') {
                // *********** Array children.
                let _p1Pick;
                log('p1DropEachChild k:', key, 'p1d:', p1DropOff, 'p1p:', p1PickOff, 'raw: -', 'p2p:', p2PickOff, 'p2d:', p2DropOff, 'op:', outPickOff, 'od:', outDropOff);
                ...
                { // Calculate _p1Pick and outPickOff
                    ...
                    while (p1pValid && typeof (p1k = p1Pick.getKey()) === 'number') {
                        ...
                        if (p1k > k1Mid || (p1k === k1Mid && (!hp || (side === LEFT && hd1))))
                            break;
                ...
                { // Calculate _p2Drop.
                    ...
                    while (p2dValid && typeof (p2dk = p2Drop.getKey()) === 'number') {
                        ...
                        const hd2 = hasDrop(c);
                        log('p2d looking at child', p2dk, c, 'at op2mid', op2Mid, 'target', k2Mid, '  / raw', raw, p2DropOff, hd2, hd1);
                        if (op2Mid > k2Mid)
                            break;
                        if (op2Mid === k2Mid) {
                            if (hd2) {
                ...
                if (writeOp1Drop(_p1Pick, p1Drop, _p2Pick, _p2Drop, w, removed2))
                    outDropOff++;
                w.ascend();
            }
    ...
    // const outDropPath = []
    const heldOutDropRead = [];
    // Its like a wave slowly breaking on the beach...
    const heldOutDropWrites = [];
    ...
    function writeHeldOp2Drop(p2Drop, outPick, outDrop, w, parentC, removedOut) {
        ...
        if (!RELEASE_MODE)
            log('writeHeldOp2Drop', 'p2Drop', p2Drop.getPath(), 'outPick', outPick && outPick.getPath(), 'outDrop', outDrop && outDrop.getPath(), 'pc', parentC, 'rm out', removedOut);
        log.prefix++;
        ...
        if (coutp && hasPick(coutp)) {
            if (coutp.p != null) {
                ...
                w = heldOutDropWrites[slot] = cursor_js_1.writeCursor();
        ...
        if (c2) {
            ...
            if ((slot2 = c2.d) != null) {
                ...
                if (_w) {
                    ...
                    w.mergeTree(_w.get());
                    outDrop = cursor_js_1.readCursor(_w.get());
                }
            }
        }
        if (!RELEASE_MODE) {
        ...
        let outPickOff = 0, outDropOff = 0;
        const oPickAdv = cursor_js_1.advancer(outPick, undefined, (k, c) => {
            if (hasPick(c)) {
                outPickOff--;
                log('outPickOff++');
            }
        });
        const oDropAdv = cursor_js_1.advancer(outDrop, (k, c) => (hasDrop(c) ? -(k - outDropOff) - 1 : k - outDropOff), (k, c) => {
            if (hasDrop(c)) {
                outDropOff++;
                log('outDropOff++');
            }
    ...
    if ((heldDropWrites.length || cancelledOp2.length) && !conflict) {
        ...
        heldDropWrites.forEach(hdw => {
            if (hdw)
                eachDrop(cursor_js_1.readCursor(hdw.get()), null, (slotOut, r) => {
                    heldOutDropRead[slotOut] = r.clone();
                });
        });
        if (!RELEASE_MODE) {
        ...
        if (!RELEASE_MODE)
            log('held output writes', heldOutDropWrites.map(h => h.get()));
        if (heldOutDropWrites.length) {
            const heldOutDropContent = heldOutDropWrites.map(w => w ? w.get() : null);
            const rOut2 = cursor_js_1.readCursor(shallowCloneOp(w.get()));
            ...
            if (heldOutDropContent.find(x => x)) {
                ...
                heldOutDropContent.forEach((data, slotOut) => {
                    if (data != null) {
                        eachDrop(cursor_js_1.readCursor(data), null, c => {
...
const opThatRemovesDE = (op) => {
    const w = cursor_js_1.writeCursor();
    cursor_js_1.readCursor(op).traverse(w, (c, w) => {
        if (hasDrop(c) || getEditType(c))
            w.write('r', true);
    });
    return w.get();
};
const resolveConflict = (conflict, side) => {
    ...
    switch (type) {
        ...
        case types_js_1.ConflictType.RM_UNEXPECTED_CONTENT:
            ...
            cursor_js_1.readCursor(op1).traverse(null, c => { if (c.r !== undefined)
                op1HasRemove = true; });
...
const normalizeConflict = ({ type, op1, op2 }) => ({ type, op1: normalize(op1), op2: normalize(op2) });
function transformWithConflictsPred(allowConflict, op1, op2, side) {
    ...
    while (true) {
        const res = tryTransform(op1, op2, side);
        if (res.ok)
            return compose(r2Aggregate, res.result);
        else {
            const { conflict } = res;
            log('detected conflict', conflict);
            if (!allowConflict(conflict))
                throwConflictErr(conflict);
            if (!RELEASE_MODE && conflict.type === types_js_1.ConflictType.BLACKHOLE) {
                const res2 = tryTransform(op2, op1, side === 'left' ? 'right' : 'left');
                // const {ok: ok2, result: result2, conflict: conflict2} = res2
                assert(!res2.ok);
                try {
                    assert(deepEqual_js_1.default(normalizeConflict(res2.conflict), normalizeConflict(invConflict(conflict))));
                }
                catch (e) {
                    ;
                    log.debug = true;
