Object.defineProperty(exports, '__esModule', { value: true });
let react = require("react");
let ts_base = require("./base");

//#region ts/shape-chart-star-icon.tsx
const element = {
	"tag": "svg",
	"attrs": {
		"xmlns": "http://www.w3.org/2000/svg",
		"fill": "none",
		"viewBox": "0 0 20 20",
		"width": "1em",
		"height": "1em"
	},
	"children": [
		{
			"tag": "path",
			"attrs": {
				"fill": "currentColor",
				"d": "M19 1V19H1V1H19ZM3 17H17V3H3V17Z"
			}
		},
		{
			"tag": "path",
			"attrs": {
				"fill": "currentColor",
				"d": "M3.64 2.22H23.64V4.22H3.64z",
				"transform": "rotate(45 3.64 2.22)"
			}
		},
		{
			"tag": "path",
			"attrs": {
				"fill": "currentColor",
				"d": "M0 0H20V2H0z",
				"transform": "rotate(135 7.72 4.5)scale(1 -1)"
			}
		},
		{
			"tag": "path",
			"attrs": {
				"fill": "currentColor",
				"d": "M11 3H25V5H11z",
				"transform": "rotate(90 11 3)"
			}
		}
	]
};
const ShapeChartStarIcon = (0, react.forwardRef)(function ShapeChartStarIcon$1(props, ref) {
	return (0, react.createElement)(ts_base.IconBase, Object.assign({}, props, {
		id: "shape-chart-star-icon",
		ref,
		icon: element
	}));
});
ShapeChartStarIcon.displayName = "ShapeChartStarIcon";
var shape_chart_star_icon_default = ShapeChartStarIcon;

//#endregion
exports.ShapeChartStarIcon = ShapeChartStarIcon;
exports.default = shape_chart_star_icon_default;