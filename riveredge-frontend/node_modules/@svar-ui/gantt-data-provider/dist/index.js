...
class RestDataProvider extends Rest {
  ...
  getHandlers() {
    return {
      ...
      "delete-task": {
        handler: async (ev) => this.sendBatch(`tasks/${ev.id}`, "DELETE")
      },
      "copy-task": {
        ignoreID: true,
        handler: async (ev) => {
      ...
      "update-link": {
        handler: async (ev) => this.sendBatch(`links/${ev.id}`, "PUT", ev.link)
      },
      "delete-link": {
        handler: async (ev) => this.sendBatch(`links/${ev.id}`, "DELETE")
      }
    };
  }
  async getData(id) {
    ...
    const [tasksResponse, links] = await Promise.all([
      this.send(id ? `tasks/${id}` : "tasks", "GET"),
      this.send(id ? `links/${id}` : "links", "GET")
    ]);
  ...
  async sendBatch(url, method, data, customHeaders) {
    if (this._batchUrl) {
      return this.sendBatchRequest(url, method, data, customHeaders);
    } else {
