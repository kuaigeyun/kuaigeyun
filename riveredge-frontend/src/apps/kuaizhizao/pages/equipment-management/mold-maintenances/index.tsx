...
const MoldMaintenancesPage: React.FC = () => {
  ...
  const [current, setCurrent] = useState<MoldMaintenance | null>(null);
  ...
  const handlePreview = async (moldId?: number, schemeId?: number) => {
    ...
    try {
      const res = await maintenancesApi.previewLines({ mold_id: moldId, scheme_id: schemeId });
  ...
  const handleEdit = async (record: MoldMaintenance) => {
    if (!record.id) return;
  ...
  const handleSubmitDoc = async (record: MoldMaintenance) => {
    if (!record.id) return;
  ...
  const handleApprove = async (record: MoldMaintenance) => {
    if (!record.id) return;
  ...
  const handleComplete = async (record: MoldMaintenance) => {
    if (!record.id) return;
        ...
        viewTypes={['table', 'help']}
          ...
          request={async (params, sort, _filter, searchFormValues) => {
            try {
              ...
              return { data: data as MoldMaintenance[], success: true, total };
            } catch {
