...
> **重要提示：** 
> - 📖 **工单和报工管理详细规范已单独剥离**：请参考《工单与报工管理详细规范.md》获取工单和报工管理的完整详细规范
> - 本文档保留工单和报工相关内容，但详细规范和强化内容请参考专门文档

> **开发状态总览：**
> - ✅ **已完成（第一步MVP）**：工单管理、报工管理、生产领料、成品入库、生产看板
> - ⏳ **已计划（第二步）**：销售预测、MRP运算、销售订单、LRP运算、采购单、采购入库、销售出库、应付应收管理、质量管理（来料/过程/成品检验）
> - ⏳ **已计划（第三步）**：系统初始化、快速配置向导、基础数据配置优化、工单高级管理、生产执行高级功能（工站上下料物料绑定）、仓储管理高级功能（装箱打包绑定、客户来料登记）、异常处理、报表分析（自研报表设计器，基于@dnd-kit + @ant-design/charts）、工位机触屏模式、工作台快捷操作、多角色场景、设备模具管理、成本核算、AI智能建议
> - ⏳ **已计划（第四步）**：二维码体系全流程应用、审批流程完善、数据验证机制、操作引导机制
> - ⏳ **已计划（第五步）**：个性化定制（自定义字段、自定义报表扩展、自定义工作流）、第三方系统集成（ERP、PLM、SCADA）、企业上下游协同
> - ⏳ **已计划（第六步）**：性能优化、功能扩展、用户体验持续优化
...
> **开发状态：** ⏳ **已计划（第三步阶段1.1）** - 系统注册、快速初始化向导、行业模板、智能默认值、AI智能建议、内置帮助系统
...
> **开发状态：** ⏳ **已计划（第三步阶段1.2）** - 组织架构、工厂建模、物料主数据、工序工艺路线、仓库配置、供应链主数据、期初数据导入
...
> **开发状态：** ⏳ **已计划（第三步阶段2.4）** - 单据节点耗时统计、单据执行效率分析、工作时间段配置
...
> **开发状态：** ⏳ **已计划（第二步、第三步）** - 单据关联展示、单据下推功能、单据上拉功能、上下游追溯
...
> **开发状态：** ⏳ **已计划（第三步阶段3.1）** - 缺料异常、交期延期异常、质量异常处理
...
> **开发状态：** ⏳ **已计划（第三步阶段3.4）** - 库存报表、生产报表、质量报表、报工统计分析、自研报表设计器（基于@dnd-kit + @ant-design/charts）
...
### 7.4 自定义报表设计器（新增，自研方案）

> **开发状态：** ⏳ **已计划（第三步阶段3.4）** - 基于@dnd-kit + @ant-design/charts的自研报表设计器

**用户操作：**
1. 进入"系统管理" → "报表管理" → "报表模板" → "新建报表模板"
2. **使用拖拽式设计器创建报表**：
   - 从组件库拖拽组件到画布（表格、图表、文本、图片等）
   - 调整组件位置和大小（拖拽调整）
   - 配置组件属性（数据源、样式、格式等）
3. **配置数据源**：
   - 选择数据源类型（API接口、数据库查询、现有报表数据）
   - 配置数据查询条件
   - 预览数据结果
4. **配置图表组件**（如：柱状图、折线图、饼图等）：
   - 选择图表类型（基于@ant-design/charts）
   - 配置数据字段映射
   - 配置图表样式（颜色、标题、图例等）
5. **配置表格组件**：
   - 选择显示字段
...
**系统行为：**
- ✅ 拖拽式设计器（基于@dnd-kit，已在kanban-board中使用）
- ✅ 组件库支持（表格、图表、文本、图片等）
- ✅ 实时预览（所见即所得）
- ✅ 数据源绑定（支持API、数据库查询）
- ✅ 报表配置存储（JSON Schema格式，存储在PostgreSQL JSONB字段）
- ✅ 报表生成引擎（openpyxl生成Excel，WeasyPrint生成PDF）
- ✅ 中文字体支持（PDF导出支持中文）
- ✅ 报表模板管理（增删改查、导入导出）
- 🤖 **AI智能建议**（通过Inngest AI工作流）：
  - 基于数据特点，建议合适的图表类型
...
**技术要点：**
- 使用@dnd-kit实现拖拽（已在项目中使用）
- 使用@ant-design/charts实现图表（已有）
- 使用openpyxl生成Excel（MIT许可证）
- 使用WeasyPrint生成PDF（BSD许可证）
- 报表配置使用JSON Schema格式存储

---

## 8. 工位机触屏模式（新增，车间固定工位核心需求）

> **开发状态：** ⏳ **已计划（第三步阶段3.8）** - 现场报工、SOP查看、图纸查看、加工程序查看
...
> **开发状态：** ⏳ **已计划（第三步阶段3.9）** - 工作台首页、快捷操作、待办事项、统计卡片
...
> **开发状态：** ⏳ **已计划（第三步、第四步）** - 数据导入导出（UniSheet）、高级搜索、打印功能、移动端支持、WebSocket实时推送、权限管理（RBAC）、审批流程
...
> **开发状态：** ⏳ **已计划（第三步阶段3.10）** - 多角色场景推演、自助式上线向导、各角色完整使用场景
...
**场景2.1：自定义报表创建（<15分钟，可选）**
1. **进入"系统管理" → "报表管理" → "报表模板" → "新建报表模板"**
2. **使用拖拽式设计器创建报表**：
   - 从组件库拖拽组件到画布（表格、图表、文本等）
   - 配置组件属性（数据源、样式等）
   - 实时预览报表效果
3. **保存报表模板**
4. **使用自定义报表模板生成报表**：
...
> **开发状态：** ⏳ **已计划（第五步）** - 自定义字段、自定义报表、自定义工作流、第三方系统集成
...
### 9.2 自定义报表（自研方案，基于@dnd-kit + @ant-design/charts）

> **开发状态：** ⏳ **已计划（第三步阶段3.4）** - 自研报表设计器，支持自定义报表模板创建

**场景：** 企业需要自定义报表格式和内容，满足特定业务需求
...
3. **配置数据源**：
   - 选择数据源类型：
     - API接口（选择系统提供的API接口）
     - 数据库查询（编写SQL查询，支持参数化查询）
     - 现有报表数据（复用其他报表的数据源）
   - 配置数据查询条件（时间范围、筛选条件等）
   - 预览数据结果（实时查看数据）
4. **配置图表组件**（如：柱状图、折线图、饼图等）：
   - 选择图表类型（基于@ant-design/charts）
   - 配置数据字段映射（X轴、Y轴、分组字段等）
   - 配置图表样式（颜色、标题、图例、坐标轴等）
   - 实时预览图表效果
5. **配置表格组件**：
   - 选择显示字段（从数据源中选择）
   - 配置字段排序和筛选
   - 配置表格样式（列宽、对齐方式、字体等）
   - 支持数据汇总（合计、平均值等）
6. **实时预览报表效果**（所见即所得，实时更新）
7. **保存报表模板**：
   - 填写报表模板名称、编码、描述
   - 设置报表模板分类
   - 保存报表配置（系统自动保存为JSON Schema格式）
8. **测试报表生成**：
   - 预览报表效果（实时预览）
   - 导出Excel测试（测试Excel格式和样式）
   - 导出PDF测试（测试PDF格式和中文字体）
9. **发布报表模板**（可选）：
   - 设置为系统模板（供其他用户使用）
   - 设置为部门模板（供部门内用户使用）
   - 设置为个人模板（仅自己使用）
...
**系统行为：**
- ✅ 拖拽式设计器（基于@dnd-kit，已在kanban-board中使用，技术栈统一）
- ✅ 组件库支持（表格、图表、文本、图片、分组等）
- ✅ 实时预览（所见即所得，实时更新）
- ✅ 数据源绑定（支持API接口、数据库查询、现有报表数据）
- ✅ 报表配置存储（JSON Schema格式，存储在PostgreSQL JSONB字段）
- ✅ 报表生成引擎：
  - Excel生成（使用openpyxl，MIT许可证，支持复杂格式、图表、样式）
  - PDF生成（使用WeasyPrint，BSD许可证，支持中文字体、HTML转PDF）
- ✅ 报表模板管理（增删改查、导入导出、模板分类）
- ✅ 报表权限控制（系统模板、部门模板、个人模板）
- ✅ 报表调度（通过Inngest定时生成报表，异步处理大报表）
- 🤖 **AI智能建议**（通过Inngest AI工作流）：
  - 基于数据特点，建议合适的图表类型
  - 检测报表配置合理性，提示优化
  - 建议报表布局优化
...
**技术要点：**
- **前端技术栈：**
  - @dnd-kit/core（拖拽，已有，已在kanban-board中使用）
  - @ant-design/charts（图表，已有，2.1.0版本）
  - recharts（图表，已有，3.5.1版本）
  - Ant Design（UI框架，已有）
  - JSON Schema（报表配置格式）
- **后端技术栈：**
  - openpyxl（Excel生成，MIT许可证）
  - WeasyPrint（PDF生成，BSD许可证）
  - PostgreSQL JSONB（报表模板存储）
  - FastAPI（API服务，已有）
  - Tortoise ORM（数据访问，已有）

**优化效果：**
- ⚡ **操作简便**：拖拽式设计，无需编写代码
- ⚡ **实时预览**：所见即所得，即时查看效果
- ⚡ **功能完整**：支持表格、图表、文本等多种组件
- ⚡ **导出灵活**：支持Excel和PDF导出，格式完整
- ⚡ **技术栈统一**：使用已有技术栈，维护成本低
...
> **开发状态：** ⏳ **已计划（第三步、第四步）** - 数据质量保障、操作引导、上线进度跟踪、系统性能优化
...
> **开发状态：** ⏳ **已计划（第四步第一阶段）** - 物料码、工单码、工序码、设备码、人员码、装箱码、追溯码
