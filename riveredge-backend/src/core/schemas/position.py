...
class PositionBase(BaseModel):
    ...
    code: str | None = Field(None, max_length=50, description="职位代码（可选，用于程序识别）")
...
class PositionUpdate(BaseModel):
    ...
    Attributes:
        ...
        is_active: 是否启用（可选）
    """
    name: str | None = Field(None, min_length=1, max_length=100, description="职位名称")
    code: str | None = Field(None, max_length=50, description="职位代码（可选，用于程序识别）")
    description: str | None = Field(None, description="职位描述")
    department_uuid: str | None = Field(None, description="所属部门UUID（可选，使用UUID而不是ID）")
...
class PositionResponse(BaseModel):
    ...
    **注意**: 遵循自增ID+UUID混合方案，只对外暴露UUID。
    ...
    code: str | None = Field(None, description="职位代码")
    description: str | None = Field(None, description="职位描述")
