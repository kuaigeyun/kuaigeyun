...
class OnlineUserResponse(BaseModel):
    ...
    login_ip: str | None = Field(None, description="登录IP")
    login_time: datetime | None = Field(None, description="登录时间")
    last_activity_time: datetime | None = Field(None, description="最后活动时间")
    session_id: str | None = Field(None, description="会话ID（JWT Token 的 jti，如果存在）")
