...
class PaymentRefundService(AppBaseService[Payment]):
    ...
    async def unconfirm_refund(
        ...
        *,
        ...
        """撤回已确认付款退款：冲回银行流水、恢复源单退款额与核销，退款单回草稿。"""
        from apps.kuaicaiwu.services.bank_account_service import BankAccountService
        from apps.kuaicaiwu.services.finance_service import AccountSettlementService
        ...
        if refund.status != "Confirmed":
            raise ValidationError("只有已确认的付款退款可以撤回确认")

        allocations = await self.get_source_payment_allocations(tenant_id, refund_payment_id)
        if not allocations:
            raise ValidationError("未关联源付款单，无法撤回确认")
        ...
        async with in_transaction():
            ...
            for row in allocations:
                ...
                await settlement_service.restore_payable_settlements_after_refund_unconfirm(
                    tenant_id,
                    source_payment_id=source_id,
                    refund_payment_id=refund_payment_id,
                    refund_amount=chunk,
    ...
    async def sync_bank_flow(
        ...
        *,
        ...
        if str(refund.settlement_type or "") != "refund":
            raise ValidationError("非退款类型付款单")
        if refund.status != "Confirmed":
            raise ValidationError("仅已确认的付款退款可补记银行流水")
