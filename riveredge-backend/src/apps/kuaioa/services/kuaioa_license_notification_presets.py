...
KUAIOA_LICENSE_NOTIFICATION_RULE_PRESETS: List[Dict[str, Any]] = [
    {
        "id": "oa_preset_compliance_license_due_soon",
        "scene_name": "证照协议到期前提醒",
        "trigger_document": DOC_COMPLIANCE_LICENSE,
        "trigger_action": ACTION_DUE_SOON,
        "template_code": "OA_COMPLIANCE_LICENSE_DUE_SOON",
        "recipient_scopes": ["user_specified", "creator"],
        "enabled": False,
    },
    {
        "id": "oa_preset_compliance_license_due_overdue",
        "scene_name": "证照协议已到期提醒",
        "trigger_document": DOC_COMPLIANCE_LICENSE,
        "trigger_action": ACTION_DUE_OVERDUE,
        "template_code": "OA_COMPLIANCE_LICENSE_DUE_OVERDUE",
        "recipient_scopes": ["user_specified", "creator"],
        "enabled": False,
    },
]
...
_TEMPLATE_CODES: Set[str] = {
    ...
    for p in KUAIOA_LICENSE_NOTIFICATION_RULE_PRESETS
...
async def load_kuaioa_license_notification_rule_presets(tenant_id: int) -> Dict[str, int]:
    ...
    created = 0
    for preset in KUAIOA_LICENSE_NOTIFICATION_RULE_PRESETS:
        ...
        if not template_uuid:
            logger.warning(
                "轻办公证照提醒预设跳过：未找到消息模板 tenant={} code={}",
                tenant_id,
                template_code,
            )
            continue
