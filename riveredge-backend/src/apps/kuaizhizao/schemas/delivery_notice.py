...
class DeliveryNoticeBase(BaseSchema):
    ...
    planned_delivery_date: date | None = Field(None, description="预计送达日期")
    carrier: str | None = Field(None, max_length=100, description="承运商/物流方式")
    tracking_number: str | None = Field(None, max_length=100, description="运单号")
...
class DeliveryNoticeCreate(DeliveryNoticeBase):
    """送货单创建schema"""
    items: list["DeliveryNoticeItemCreate"] | None = Field(None, description="通知明细列表")
...
class DeliveryNoticeItemBase(BaseSchema):
    ...
    delivery_item_id: int | None = Field(None, description="销售出库明细ID")
    notes: str | None = Field(None, description="备注")
