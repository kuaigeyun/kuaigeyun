...
class ConvertToPurchaseOrderRequest(BaseModel):
    ...
    item_unit_prices: Optional[Dict[int, float]] = Field(
        None,
        description="按 item_id 覆盖单价，不传则用申请行 suggested_unit_price",
    )
    ...
    @field_validator("item_quantities", mode="before")
    @classmethod
    def _coerce_item_quantity_keys(cls, v):
        if v is None:
            return v
    ...
    @field_validator("item_unit_prices", mode="before")
    @classmethod
    def _coerce_item_unit_price_keys(cls, v):
        if v is None:
            return v
        if not isinstance(v, dict):
            return v
