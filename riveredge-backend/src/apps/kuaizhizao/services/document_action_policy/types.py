...
CAPABILITY_REASON_MESSAGES: dict[str, str] = {
    ...
    "quotation.convert_contract.removed": "报价单已不再下推销售合同，请直接下推销售订单",
    ...
    "sales_contract.push_to_work_order.removed": "框架合同不再直推工单，请先释放销售订单",
    ...
    "work_order.release.pending_audit": "工单待审核，通过后方可下达",
    "work_order.release.audit_rejected": "工单审核已驳回，请修改后重新提交审核",
    "work_order.release.audit_required": "请先提交并通过工单审核后再下达",
    ...
    "work_order.push_production_picking.not_allowed": "仅「已下达」或「执行中」的工单可下推生产领料，当前状态不符合",
    ...
    "work_order.push_production_picking.pending_picking": "该工单已有待处理的生产领料单，请先完成或取消后再下推",
