...
class DefectTypeUpdate(BaseModel):
    ...
    code: str | None = Field(None, max_length=50, description="不良品编码")
    name: str | None = Field(None, max_length=200, description="不良品名称")
    category: str | None = Field(None, max_length=50, description="分类")
    description: str | None = Field(None, description="描述")
...
class OperationCreate(OperationBase):
    """创建工序 Schema"""
    defect_type_uuids: list[str] | None = Field(None, alias="defectTypeUuids", description="允许绑定的不良品项 UUID 列表")
    default_operator_uuids: list[str] | None = Field(None, alias="defaultOperatorUuids", description="默认生产人员（用户UUID列表）")
    model_config = ConfigDict(populate_by_name=True)
...
class OperationUpdate(BaseModel):
    ...
    code: str | None = Field(None, max_length=50, description="工序编码")
    name: str | None = Field(None, max_length=200, description="工序名称")
    description: str | None = Field(None, description="描述")
    reporting_type: str | None = Field(None, alias="reportingType", max_length=20, description="报工类型（quantity:按数量报工, status:按状态报工）")
    allow_jump: bool | None = Field(None, alias="allowJump", description="是否允许跳转（true:允许跳转，不依赖上道工序完成, false:不允许跳转，必须完成上道工序）")
    is_active: bool | None = Field(None, alias="isActive", description="是否启用")
    defect_type_uuids: list[str] | None = Field(None, alias="defectTypeUuids", description="允许绑定的不良品项 UUID 列表")
    default_operator_uuids: list[str] | None = Field(None, alias="defaultOperatorUuids", description="默认生产人员（用户UUID列表）")
    model_config = ConfigDict(populate_by_name=True)
    ...
    @field_validator("reporting_type")
    @classmethod
    def validate_reporting_type(cls, v: str | None) -> str | None:
        """验证报工类型"""
...
class ProcessRouteBase(BaseModel):
    ...
    base_version: str | None = Field(None, max_length=20, description="基于版本（从哪个版本创建）")
    effective_date: datetime | None = Field(None, description="生效日期")
    operation_sequence: dict[str, Any] | None = Field(None, description="工序序列（JSON格式，支持子工艺路线）")
    parent_route_uuid: str | None = Field(None, description="父工艺路线UUID（如果此工艺路线是子工艺路线）")
    parent_operation_uuid: str | None = Field(None, max_length=100, description="父工序UUID（此子工艺路线所属的父工序）")
...
class ProcessRouteUpdate(BaseModel):
    ...
    code: str | None = Field(None, max_length=50, description="工艺路线编码")
    name: str | None = Field(None, max_length=200, description="工艺路线名称")
    description: str | None = Field(None, description="描述")
    version: str | None = Field(None, max_length=20, description="版本号")
    version_description: str | None = Field(None, description="版本说明")
    base_version: str | None = Field(None, max_length=20, description="基于版本")
    effective_date: datetime | None = Field(None, description="生效日期")
    operation_sequence: dict[str, Any] | None = Field(None, description="工序序列（JSON格式）")
...
class ProcessRouteResponse(ProcessRouteBase):
    ...
    parent_route_uuid: str | None = Field(None, description="父工艺路线UUID")
...
class ProcessRouteVersionCreate(BaseModel):
    ...
    version_description: str | None = Field(None, description="版本说明")
    effective_date: datetime | None = Field(None, description="生效日期（可选，默认为当前日期）")
...
class ProcessRouteTemplateBase(BaseModel):
    ...
    category: str | None = Field(None, max_length=50, description="模板分类（如：注塑类、组装类、包装类等）")
    ...
    scope_groups: list[str] | None = Field(None, description="适用范围物料分组UUID列表（当scope为specific_groups时使用）")
    process_route_config: dict[str, Any] | None = Field(None, description="工艺路线配置（JSON格式）")
...
class ProcessRouteTemplateUpdate(BaseModel):
    ...
    scope_groups: list[str] | None = Field(None, description="适用范围物料分组UUID列表")
    process_route_config: dict[str, Any] | None = Field(None, description="工艺路线配置")
    is_active: bool | None = Field(None, description="是否启用")
...
class ProcessRouteTemplateResponse(ProcessRouteTemplateBase):
    ...
    base_version: str | None = Field(None, description="基于版本")
    created_at: datetime = Field(..., description="创建时间")
    updated_at: datetime = Field(..., description="更新时间")
    deleted_at: datetime | None = Field(None, description="删除时间")
...
class SOPBase(BaseModel):
    ...
    operation_id: int | None = Field(None, description="关联工序ID")
    version: str | None = Field(None, max_length=20, description="版本号")
    content: str | None = Field(None, description="SOP内容（支持富文本）")
    attachments: dict[str, Any] | None = Field(None, description="附件列表（JSON格式）")
    flow_config: dict[str, Any] | None = Field(None, description="流程配置（ProFlow JSON格式，作业指导步骤与顺序）")
    form_config: dict[str, Any] | None = Field(None, description="报工数据采集项（Formily Schema，数字/选择/文本/日期时间及必填与校验）")
    is_active: bool = Field(True, description="是否启用")
    # 阶段一：绑定与融合
    material_group_uuids: list[str] | None = Field(None, description="绑定的物料组 UUID 列表")
    material_uuids: list[str] | None = Field(None, description="绑定的具体物料 UUID 列表，匹配时优先于物料组")
    route_uuids: list[str] | None = Field(None, description="载入的工艺路线 UUID 列表，作为融合输入")
    bom_load_mode: str | None = Field("by_material", max_length=32, description="BOM载入方式：by_material/by_material_group/specific_bom")
    specific_bom_uuid: str | None = Field(None, max_length=36, description="指定BOM的UUID（bom_load_mode=specific_bom时使用）")
...
class SOPBatchCreateFromRouteRequest(BaseModel):
    ...
    material_uuids: list[str] | None = Field(None, description="绑定的物料UUID列表（可选）")
    material_group_uuids: list[str] | None = Field(None, description="绑定的物料组UUID列表（可选）")


class SOPUpdate(BaseModel):
    """更新作业程序（SOP） Schema"""
    
    code: str | None = Field(None, max_length=50, description="SOP编码")
    name: str | None = Field(None, max_length=200, description="SOP名称")
    operation_id: int | None = Field(None, description="关联工序ID")
    version: str | None = Field(None, max_length=20, description="版本号")
    content: str | None = Field(None, description="SOP内容（支持富文本）")
    attachments: dict[str, Any] | None = Field(None, description="附件列表（JSON格式）")
    flow_config: dict[str, Any] | None = Field(None, description="流程配置（ProFlow JSON格式）")
    form_config: dict[str, Any] | None = Field(None, description="报工数据采集项（Formily Schema）")
    is_active: bool | None = Field(None, description="是否启用")
    material_group_uuids: list[str] | None = Field(None, description="绑定的物料组 UUID 列表")
    material_uuids: list[str] | None = Field(None, description="绑定的具体物料 UUID 列表")
    route_uuids: list[str] | None = Field(None, description="载入的工艺路线 UUID 列表")
    bom_load_mode: str | None = Field(None, max_length=32, description="BOM载入方式")
    specific_bom_uuid: str | None = Field(None, max_length=36, description="指定BOM的UUID")
