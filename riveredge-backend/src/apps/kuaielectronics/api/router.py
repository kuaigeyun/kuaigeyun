...
from apps.kuaielectronics.services import label_oem_seed_service as _label_oem_hooks  # noqa: F401

router = APIRouter(prefix="", tags=["App - KuaiElectronics"])
...
@router.get(
    ...
    dependencies=[
        Depends(
            require_access(
                "kuaielectronics.entry",
                "read",
                required_permissions=["kuaielectronics:entry:read"],
            )
        )
    ],
)
async def health(tenant_id: int = Depends(get_current_tenant)) -> dict:
    return {"ok": True, "app": "kuaielectronics", "tenant_id": tenant_id}
...
@router.get(
    "/esd/catalog",
    summary="ESD 项目清单与方案",
    dependencies=[Depends(require_permission_codes("kuaielectronics:esd:read"))],
)
async def get_esd_catalog(tenant_id: int = Depends(get_current_tenant)) -> Dict[str, Any]:
    from apps.kuaielectronics.services.esd_seed_service import get_esd_catalog_summary
...
@router.put(
    ...
    dependencies=[Depends(require_permission_codes("kuaielectronics:esd:update"))],
)
async def update_esd_catalog(
    body: EsdCatalogUpdateRequest,
    tenant_id: int = Depends(get_current_tenant),
) -> Dict[str, Any]:
    from apps.kuaielectronics.services.esd_seed_service import (
        ensure_esd_catalog,
        get_esd_profile,
    )
...
@router.get(
    ...
    dependencies=[Depends(require_permission_codes("kuaielectronics:label-oem:read"))],
)
async def list_label_oem_packs(tenant_id: int = Depends(get_current_tenant)) -> Dict[str, Any]:
    from apps.kuaielectronics.services.label_oem_seed_service import get_oem_pack_status
...
@router.post(
    ...
    dependencies=[Depends(require_permission_codes("kuaielectronics:label-oem:update"))],
)
async def ensure_label_oem_packs(
    ...
    from apps.kuaielectronics.services.label_oem_seed_service import ensure_oem_label_packs
