...
router = APIRouter(prefix="/config", tags=["App - Kuaiplm - 配置"])
...
@router.post(
    ...
    summary="加载快研发消息提醒规则预设",
    dependencies=[Depends(require_permission_codes("system:message-template:create"))],
)
async def load_kuaiplm_notification_rule_presets_api(
    tenant_id: Annotated[int, Depends(get_current_tenant)],
    _: Annotated[User, Depends(get_current_user)],
):
    result = await load_kuaiplm_notification_rule_presets(tenant_id)
