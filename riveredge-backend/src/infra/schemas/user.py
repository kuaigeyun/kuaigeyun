...
class UserBase(BaseModel):
    ...
    source: str | None = Field(None, max_length=50, description="用户来源（invite_code, personal, organization等）")
...
class UserCreate(UserBase):
    ...
    Attributes:
        ...
        is_infra_admin: 是否为平台管理（默认 False）
...
class UserUpdate(BaseModel):
    ...
    Attributes:
        ...
        full_name: 用户全名（可选）
        is_active: 是否激活（可选）
        is_infra_admin: 是否为平台管理（可选）
        is_tenant_admin: 是否为组织管理员（可选）
    """
    
    username: str | None = Field(None, min_length=3, max_length=50, description="用户名（3-50 字符）")
    email: EmailStr | None = Field(None, description="用户邮箱（邮箱格式）")
    password: str | None = Field(None, min_length=8, max_length=100, description="密码（最少 8 字符）")
    full_name: str | None = Field(None, max_length=100, description="用户全名（可选）")
    is_active: bool | None = Field(None, description="是否激活")
    is_infra_admin: bool | None = Field(None, description="是否为平台管理（系统级超级管理员，需 tenant_id=None）")
    is_tenant_admin: bool | None = Field(None, description="是否为组织管理员")
