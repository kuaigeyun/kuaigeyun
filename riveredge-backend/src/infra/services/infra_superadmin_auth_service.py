...
from infra.schemas.infra_superadmin import InfraSuperAdminLoginRequest
...
class InfraSuperAdminAuthService:
    """
    平台超级管理员认证服务类
    ...
    async def login(
        ...
        admin = await service.verify_infra_superadmin_credentials(
            data.username,
            data.password
        )
        ...
        token_info = create_token_for_infra_superadmin(admin)
        ...
        return {
            **token_info,
            "user": InfraSuperAdminResponse.model_validate(admin).model_dump(),
