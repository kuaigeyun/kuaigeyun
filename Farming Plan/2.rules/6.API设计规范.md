# RiverEdge SaaS 多租户框架 - API 设计规范

## 📋 概述

本文档定义 **RiverEdge SaaS 多租户框架 (RiverEdge SaaS Multi-tenant Framework)** 的**统一 API 设计规范**，确保前后端协作顺畅，接口风格一致，提升开发效率和代码可维护性。

**创建日期**：2025-11-17

## 🎯 设计原则

1. **RESTful 风格**：遵循 REST 架构风格，使用标准 HTTP 方法
2. **统一响应格式**：所有 API 返回统一的响应结构
3. **版本控制**：API 路径包含版本号（`/api/v1/`）
4. **多租户隔离**：所有 API 自动处理租户隔离，无需手动传递 `tenant_id`
5. **错误处理统一**：使用标准 HTTP 状态码和业务错误码
6. **文档自动生成**：利用 FastAPI 自动生成 OpenAPI/Swagger 文档

## 📐 API 路径规范

### 路径前缀

**规则**：所有 API 路径必须以 `/api/v1/` 开头

```
/api/v1/{resource}
```

**示例**：
- ✅ `/api/v1/users` - 用户管理 API
- ✅ `/api/v1/tenants` - 租户管理 API
- ✅ `/api/v1/auth/login` - 认证 API
- ❌ `/api/users` - 错误：缺少版本号
- ❌ `/v1/users` - 错误：缺少 `/api` 前缀

### 资源命名

**规则**：使用复数形式，小写字母，多个单词使用连字符（`-`）分隔

**示例**：
- ✅ `/api/v1/users` - 用户资源
- ✅ `/api/v1/order-items` - 订单明细资源
- ✅ `/api/v1/system-configs` - 系统配置资源
- ❌ `/api/v1/user` - 错误：应使用复数
- ❌ `/api/v1/orderItems` - 错误：应使用连字符分隔

### 嵌套资源

**规则**：使用路径参数表示资源关系

**示例**：
- ✅ `/api/v1/users/{user_id}/orders` - 获取用户的订单列表
- ✅ `/api/v1/tenants/{tenant_id}/users` - 获取租户的用户列表
- ❌ `/api/v1/users/orders?user_id=123` - 错误：应使用路径参数

## 🔧 HTTP 方法使用规范

### GET - 查询资源

**使用场景**：
- 获取资源列表
- 获取单个资源详情
- 查询操作（不修改数据）

**示例**：
```python
# 获取用户列表
GET /api/v1/users?page=1&page_size=10

# 获取单个用户
GET /api/v1/users/{user_id}

# 查询操作
GET /api/v1/users/search?keyword=admin
```

### POST - 创建资源

**使用场景**：
- 创建新资源
- 执行操作（如登录、导出等）

**示例**：
```python
# 创建用户
POST /api/v1/users
Body: { "username": "admin", "email": "admin@example.com" }

# 用户登录
POST /api/v1/auth/login
Body: { "username": "admin", "password": "123456" }
```

### PUT - 完整更新资源

**使用场景**：
- 完整更新资源（替换所有字段）

**示例**：
```python
# 完整更新用户
PUT /api/v1/users/{user_id}
Body: { "username": "admin", "email": "admin@example.com", "status": "active" }
```

### PATCH - 部分更新资源

**使用场景**：
- 部分更新资源（只更新提供的字段）

**示例**：
```python
# 部分更新用户
PATCH /api/v1/users/{user_id}
Body: { "email": "newemail@example.com" }
```

### DELETE - 删除资源

**使用场景**：
- 删除单个资源
- 批量删除资源

**示例**：
```python
# 删除单个用户
DELETE /api/v1/users/{user_id}

# 批量删除用户
DELETE /api/v1/users
Body: { "ids": [1, 2, 3] }
```

## 📦 统一响应格式

### 成功响应

**标准格式**：
```json
{
  "code": 200,
  "message": "success",
  "data": { ... },
  "timestamp": "2025-11-17T10:00:00Z"
}
```

**分页响应**：
```json
{
  "code": 200,
  "message": "success",
  "data": {
    "items": [ ... ],
    "total": 100,
    "page": 1,
    "page_size": 10,
    "total_pages": 10
  },
  "timestamp": "2025-11-17T10:00:00Z"
}
```

**列表响应（无分页）**：
```json
{
  "code": 200,
  "message": "success",
  "data": [ ... ],
  "timestamp": "2025-11-17T10:00:00Z"
}
```

### 错误响应

**标准格式**：
```json
{
  "code": 400,
  "message": "请求参数错误",
  "detail": "用户名不能为空",
  "timestamp": "2025-11-17T10:00:00Z"
}
```

**验证错误响应**：
```json
{
  "code": 422,
  "message": "数据验证失败",
  "detail": [
    {
      "field": "email",
      "message": "邮箱格式不正确"
    },
    {
      "field": "password",
      "message": "密码长度至少 8 位"
    }
  ],
  "timestamp": "2025-11-17T10:00:00Z"
}
```

### 响应字段说明

| 字段 | 类型 | 说明 | 必填 |
|------|------|------|------|
| `code` | number | HTTP 状态码或业务错误码 | ✅ |
| `message` | string | 响应消息 | ✅ |
| `data` | any | 响应数据（成功时） | ❌ |
| `detail` | string/array | 错误详情（错误时） | ❌ |
| `timestamp` | string | 响应时间戳（ISO 8601） | ✅ |

## 🔢 HTTP 状态码规范

### 标准 HTTP 状态码

| 状态码 | 说明 | 使用场景 |
|--------|------|----------|
| `200` | OK | 请求成功 |
| `201` | Created | 资源创建成功 |
| `204` | No Content | 删除成功，无返回内容 |
| `400` | Bad Request | 请求参数错误 |
| `401` | Unauthorized | 未授权（未登录或 Token 无效） |
| `403` | Forbidden | 权限不足 |
| `404` | Not Found | 资源不存在 |
| `409` | Conflict | 资源冲突（如用户名已存在） |
| `422` | Unprocessable Entity | 数据验证失败 |
| `500` | Internal Server Error | 服务器内部错误 |

### 业务错误码（可选）

**说明**：在 `code` 字段中可以使用业务错误码，但必须配合 HTTP 状态码使用

**示例**：
```json
{
  "code": 10001,  // 业务错误码
  "message": "用户不存在",
  "detail": "用户 ID 123 不存在"
}
```

**HTTP 状态码映射**：
- `400` + `10001-10099`: 通用业务错误
- `401` + `10101-10199`: 认证相关错误
- `403` + `10201-10299`: 权限相关错误
- `404` + `10301-10399`: 资源不存在错误
- `409` + `10401-10499`: 资源冲突错误

## 📄 请求参数规范

### Query 参数（查询参数）

**使用场景**：
- 分页参数（`page`, `page_size`）
- 排序参数（`sort`, `order`）
- 筛选参数（`filter`, `search`）
- 查询条件（`keyword`, `status` 等）

**示例**：
```
GET /api/v1/users?page=1&page_size=10&sort=created_at&order=desc&status=active&keyword=admin
```

### Path 参数（路径参数）

**使用场景**：
- 资源 ID
- 嵌套资源标识

**规则**：
- 使用 `snake_case` 命名
- 必须包含在路径中

**示例**：
```
GET /api/v1/users/{user_id}
GET /api/v1/users/{user_id}/orders/{order_id}
```

### Body 参数（请求体）

**使用场景**：
- POST 创建资源
- PUT 完整更新资源
- PATCH 部分更新资源
- 批量操作

**规则**：
- 使用 JSON 格式
- 遵循 Pydantic Schema 定义

**示例**：
```json
POST /api/v1/users
{
  "username": "admin",
  "email": "admin@example.com",
  "password": "123456",
  "status": "active"
}
```

## 📊 分页规范

### 分页参数

**标准参数**：
- `page`: 页码（从 1 开始，默认 1）
- `page_size`: 每页数量（默认 10，最大 100）

**示例**：
```
GET /api/v1/users?page=1&page_size=20
```

### 分页响应

**标准格式**：
```json
{
  "code": 200,
  "message": "success",
  "data": {
    "items": [ ... ],
    "total": 100,
    "page": 1,
    "page_size": 20,
    "total_pages": 5
  }
}
```

**字段说明**：
- `items`: 当前页数据列表
- `total`: 总记录数
- `page`: 当前页码
- `page_size`: 每页数量
- `total_pages`: 总页数（可选，前端可计算）

## 🔍 排序和筛选规范

### 排序参数

**标准参数**：
- `sort`: 排序字段（默认 `created_at`）
- `order`: 排序方向（`asc` 或 `desc`，默认 `desc`）

**示例**：
```
GET /api/v1/users?sort=username&order=asc
GET /api/v1/users?sort=created_at&order=desc
```

**多字段排序**（可选）：
```
GET /api/v1/users?sort=status,created_at&order=asc,desc
```

### 筛选参数

**标准参数**：
- `filter`: 筛选条件（JSON 字符串或查询参数）
- `search`: 模糊搜索关键词

**示例**：
```
# 方式 1: 使用查询参数
GET /api/v1/users?status=active&role=admin

# 方式 2: 使用 filter 参数（JSON 字符串）
GET /api/v1/users?filter={"status":"active","role":"admin"}

# 方式 3: 使用 search 参数（模糊搜索）
GET /api/v1/users?search=admin
```

**日期范围筛选**：
```
GET /api/v1/users?created_at_start=2025-01-01&created_at_end=2025-12-31
```

## 🏢 多租户 API 规范

### 租户隔离原则

**核心原则**：
1. **所有数据模型必须包含 `tenant_id` 字段**
2. **所有查询必须自动过滤租户**
3. **JWT Token 必须包含租户信息**
4. **API 请求自动从 Token 获取租户信息**

### 租户信息获取

**后端实现**：
```python
from fastapi import Depends
from core.dependencies import get_current_tenant

@router.get("/users")
async def get_users(
    tenant_id: int = Depends(get_current_tenant)
):
    # tenant_id 自动从 JWT Token 获取
    # 查询自动过滤租户
    users = await User.filter(tenant_id=tenant_id).all()
    return users
```

**前端实现**：
```typescript
// JWT Token 自动包含租户信息
// 前端无需手动传递 tenant_id
const users = await getUserList({ page: 1, page_size: 10 });
```

### 租户管理 API

**超级管理员 API**（需要特殊权限）：
```
GET /api/v1/admin/tenants - 获取所有租户列表
POST /api/v1/admin/tenants - 创建租户
PUT /api/v1/admin/tenants/{tenant_id} - 更新租户
DELETE /api/v1/admin/tenants/{tenant_id} - 删除租户
```

**租户注册 API**（公开 API）：
```
POST /api/v1/auth/register - 租户注册
Body: {
  "tenant_name": "示例公司",
  "username": "admin",
  "email": "admin@example.com",
  "password": "123456"
}
```

### 禁止事项

**❌ 禁止在 API 路径中传递 `tenant_id`**：
```
# ❌ 错误：不应在路径中传递 tenant_id
GET /api/v1/tenants/{tenant_id}/users
```

**❌ 禁止在 Query 参数中传递 `tenant_id`**：
```
# ❌ 错误：不应在查询参数中传递 tenant_id
GET /api/v1/users?tenant_id=1
```

**✅ 正确：自动从 JWT Token 获取**：
```
# ✅ 正确：tenant_id 自动从 Token 获取
GET /api/v1/users
```

## 📝 代码示例

### 后端实现（FastAPI）

**统一响应模型**：
```python
from typing import Generic, TypeVar, Optional, List
from pydantic import BaseModel
from datetime import datetime

T = TypeVar('T')

class ResponseModel(BaseModel, Generic[T]):
    """统一响应模型"""
    code: int = 200
    message: str = "success"
    data: Optional[T] = None
    timestamp: datetime = datetime.now()

class PageResponse(BaseModel, Generic[T]):
    """分页响应模型"""
    items: List[T]
    total: int
    page: int
    page_size: int
    total_pages: int

class ListResponse(BaseModel, Generic[T]):
    """列表响应模型"""
    items: List[T]
```

**API 路由示例**：
```python
from fastapi import APIRouter, Depends, Query
from core.response import ResponseModel, PageResponse
from core.dependencies import get_current_tenant
from schemas.user import UserResponse, UserCreate, UserUpdate
from services.user_service import UserService

router = APIRouter(prefix="/api/v1/users", tags=["users"])

@router.get("", response_model=ResponseModel[PageResponse[UserResponse]])
async def get_users(
    page: int = Query(1, ge=1),
    page_size: int = Query(10, ge=1, le=100),
    sort: str = Query("created_at"),
    order: str = Query("desc", regex="^(asc|desc)$"),
    keyword: Optional[str] = Query(None),
    tenant_id: int = Depends(get_current_tenant)
):
    """获取用户列表"""
    users, total = await UserService.get_list(
        page=page,
        page_size=page_size,
        sort=sort,
        order=order,
        keyword=keyword,
        tenant_id=tenant_id
    )
    return ResponseModel(
        data=PageResponse(
            items=users,
            total=total,
            page=page,
            page_size=page_size,
            total_pages=(total + page_size - 1) // page_size
        )
    )

@router.post("", response_model=ResponseModel[UserResponse], status_code=201)
async def create_user(
    data: UserCreate,
    tenant_id: int = Depends(get_current_tenant)
):
    """创建用户"""
    user = await UserService.create(data, tenant_id=tenant_id)
    return ResponseModel(data=user, code=201)

@router.get("/{user_id}", response_model=ResponseModel[UserResponse])
async def get_user(
    user_id: int,
    tenant_id: int = Depends(get_current_tenant)
):
    """获取用户详情"""
    user = await UserService.get_by_id(user_id, tenant_id=tenant_id)
    if not user:
        raise HTTPException(status_code=404, detail="用户不存在")
    return ResponseModel(data=user)

@router.put("/{user_id}", response_model=ResponseModel[UserResponse])
async def update_user(
    user_id: int,
    data: UserUpdate,
    tenant_id: int = Depends(get_current_tenant)
):
    """更新用户"""
    user = await UserService.update(user_id, data, tenant_id=tenant_id)
    if not user:
        raise HTTPException(status_code=404, detail="用户不存在")
    return ResponseModel(data=user)

@router.delete("/{user_id}", status_code=204)
async def delete_user(
    user_id: int,
    tenant_id: int = Depends(get_current_tenant)
):
    """删除用户"""
    success = await UserService.delete(user_id, tenant_id=tenant_id)
    if not success:
        raise HTTPException(status_code=404, detail="用户不存在")
    return Response(status_code=204)
```

### 前端实现（TypeScript + Umi Request）

**类型定义**：
```typescript
// types/api.ts
export interface ResponseModel<T> {
  code: number;
  message: string;
  data?: T;
  timestamp: string;
}

export interface PageResponse<T> {
  items: T[];
  total: number;
  page: number;
  page_size: number;
  total_pages: number;
}

export interface ListResponse<T> {
  items: T[];
}
```

**API 服务示例**：
```typescript
// services/userService.ts
import { request } from '@umijs/max';
import type { ResponseModel, PageResponse } from '@/types/api';
import type { User, UserCreate, UserUpdate } from '@/types/user';

/**
 * 获取用户列表
 * 
 * @param params - 查询参数
 * @returns 用户列表响应
 */
export async function getUserList(params: {
  page?: number;
  page_size?: number;
  sort?: string;
  order?: 'asc' | 'desc';
  keyword?: string;
}): Promise<ResponseModel<PageResponse<User>>> {
  return request('/api/v1/users', {
    method: 'GET',
    params,
  });
}

/**
 * 获取用户详情
 * 
 * @param user_id - 用户 ID
 * @returns 用户详情响应
 */
export async function getUser(user_id: number): Promise<ResponseModel<User>> {
  return request(`/api/v1/users/${user_id}`, {
    method: 'GET',
  });
}

/**
 * 创建用户
 * 
 * @param data - 用户创建数据
 * @returns 创建的用户
 */
export async function createUser(data: UserCreate): Promise<ResponseModel<User>> {
  return request('/api/v1/users', {
    method: 'POST',
    data,
  });
}

/**
 * 更新用户
 * 
 * @param user_id - 用户 ID
 * @param data - 用户更新数据
 * @returns 更新的用户
 */
export async function updateUser(
  user_id: number,
  data: UserUpdate
): Promise<ResponseModel<User>> {
  return request(`/api/v1/users/${user_id}`, {
    method: 'PUT',
    data,
  });
}

/**
 * 删除用户
 * 
 * @param user_id - 用户 ID
 */
export async function deleteUser(user_id: number): Promise<void> {
  return request(`/api/v1/users/${user_id}`, {
    method: 'DELETE',
  });
}
```

**组件使用示例**：
```typescript
// pages/UserList/index.tsx
import { useState, useEffect } from 'react';
import { ProTable } from '@ant-design/pro-components';
import { getUserList, deleteUser } from '@/services/userService';
import { message } from 'antd';

export default function UserList() {
  const [dataSource, setDataSource] = useState([]);
  const [loading, setLoading] = useState(false);

  const fetchUsers = async (params: any) => {
    setLoading(true);
    try {
      const response = await getUserList(params);
      if (response.code === 200 && response.data) {
        setDataSource(response.data.items);
        return {
          data: response.data.items,
          total: response.data.total,
          success: true,
        };
      }
      return { data: [], total: 0, success: false };
    } catch (error: any) {
      message.error(error.message || '获取用户列表失败');
      return { data: [], total: 0, success: false };
    } finally {
      setLoading(false);
    }
  };

  const handleDelete = async (user_id: number) => {
    try {
      await deleteUser(user_id);
      message.success('删除成功');
      fetchUsers({ page: 1, page_size: 10 });
    } catch (error: any) {
      message.error(error.message || '删除失败');
    }
  };

  return (
    <ProTable
      request={fetchUsers}
      columns={[
        { title: 'ID', dataIndex: 'id', key: 'id' },
        { title: '用户名', dataIndex: 'username', key: 'username' },
        { title: '邮箱', dataIndex: 'email', key: 'email' },
        {
          title: '操作',
          key: 'action',
          render: (_, record) => (
            <a onClick={() => handleDelete(record.id)}>删除</a>
          ),
        },
      ]}
      rowKey="id"
      pagination={{
        pageSize: 10,
        showSizeChanger: true,
      }}
    />
  );
}
```

## 🔐 认证和授权

### JWT Token 规范

**Token 结构**：
```json
{
  "user_id": 1,
  "tenant_id": 1,
  "username": "admin",
  "roles": ["admin"],
  "permissions": ["user:read", "user:write"],
  "exp": 1734422400
}
```

**Token 传递**：
- **Header**: `Authorization: Bearer {token}`
- **Cookie**: `token={token}`（可选）

### 认证 API

**登录**：
```
POST /api/v1/auth/login
Body: {
  "username": "admin",
  "password": "123456"
}
Response: {
  "code": 200,
  "message": "success",
  "data": {
    "token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...",
    "user": { ... }
  }
}
```

**刷新 Token**：
```
POST /api/v1/auth/refresh
Headers: {
  "Authorization": "Bearer {refresh_token}"
}
```

**登出**：
```
POST /api/v1/auth/logout
Headers: {
  "Authorization": "Bearer {token}"
}
```

## 📚 相关文档

- [2.字段命名规范.md](./2.字段命名规范.md) - 命名规范
- [3.数据库命名规范.md](./3.数据库命名规范.md) - 数据库命名规范
- [4.注释规范.md](./4.注释规范.md) - 注释规范
- [AGENTS.md](./AGENTS.md) - AI 助手开发规范
- [1.最终技术选型.md](../1.plan/1.最终技术选型.md) - 技术选型
- [2.架构设计文档.md](../1.plan/2.架构设计文档.md) - 架构设计

## ✅ 检查清单

在开发 API 时，请确保：

- [ ] API 路径以 `/api/v1/` 开头
- [ ] 使用正确的 HTTP 方法（GET, POST, PUT, PATCH, DELETE）
- [ ] 返回统一的响应格式
- [ ] 使用正确的 HTTP 状态码
- [ ] 分页参数使用 `page` 和 `page_size`
- [ ] 排序参数使用 `sort` 和 `order`
- [ ] 多租户隔离自动处理（不手动传递 `tenant_id`）
- [ ] JWT Token 自动包含租户信息
- [ ] 错误响应格式统一
- [ ] API 文档自动生成（FastAPI OpenAPI）

---

**最后更新**：2025-11-17

