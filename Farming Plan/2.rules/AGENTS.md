# RiverEdge SaaS 多租户框架 - AI 助手开发规范
...
3. ✅ 检查 FastAPI 生态
   → 发现 Pydantic（FastAPI 官方推荐）
   → 使用 Pydantic ✅

结果：使用 Pydantic 进行数据验证
```
...
- **编程语言**: Python 3.13+
- **Web 框架**: FastAPI 0.115+（**不得使用 Flask、Django 等**）
- **数据验证**: Pydantic v2.9+（**不得使用其他验证库**）
- **数据库**: PostgreSQL 15+（**不得使用 MySQL、MongoDB 等**）
- **ORM 框架**: Tortoise ORM 0.20+（**不得使用 SQLAlchemy、Django ORM 等**）
...
- **核心框架**: React 19.2+（**不得使用 Vue、Angular 等**）
- **应用框架**: Umi V4 (@umijs/max)（**不得使用 Next.js、Create React App 等**）
- **UI 组件库**: Ant Design v6 + Ant Design Pro Components 2.x（**不得使用 Material-UI、Element UI 等**）
- **状态管理**: Umi Model（**不得使用 Redux、Zustand 等**）
- **类型系统**: TypeScript 5.6+（**必须使用 TypeScript，不得使用 JavaScript**）
- **HTTP 客户端**: Umi Request（**不得使用 Axios、fetch 等**）
- **路由**: React Router 6 (Umi 内置)（**不得使用其他路由库**）
- **构建工具**: Vite (Umi 内置)（**不得使用 Webpack 等**）
- **图表库**: @ant-design/charts（**不得使用 ECharts、Chart.js 等**）
- **表单自定义字段**: Formily (@formily/core + @formily/react + @formily/antd-v5)（**不得使用其他表单库**）
...
router = APIRouter(prefix="/{api_path}", tags=["{ModelName}"])
...
// ✅ 正确：使用 Umi Request 统一错误处理
// 在 request 拦截器中统一处理错误
```
...
10. [6.API设计规范.md](./6.API设计规范.md) - ⭐ **API 设计规范（统一响应格式、RESTful 风格、多租户规范）**
