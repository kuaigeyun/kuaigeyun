...
- ❌ **绝对禁止使用 PowerShell 批量修改文件**（极易产生中文乱码问题）
  ...
  - 禁止使用 PowerShell 的字符串操作批量修改文件编码
  - **为什么无法彻底解决**：
  ...
  - **推荐替代方案**：
    - ✅ **使用 Python 脚本进行批量操作**（推荐，Python 3 默认 UTF-8，编码处理稳定）
    - ✅ **使用专业的文本编辑器**（如 VS Code、Notepad++，支持批量查找替换，UTF-8 编码安全）
    - ✅ **直接使用文件编辑工具逐个修改**（最安全）
    - ⚠️ **使用 Git 工具进行批量替换**（如 `git grep` + 手动替换，需注意编码）
  - **如果必须使用 PowerShell**（不推荐）：
    - 必须显式指定 UTF-8 编码：`Get-Content -Encoding UTF8`、`Set-Content -Encoding UTF8`
    - 设置代码页：`chcp 65001`
    - 设置输出编码：`$OutputEncoding = [System.Text.Encoding]::UTF8`
    - **即使做了这些设置，仍无法保证在所有场景下都稳定，风险自负**
...
- [ ] 文件编码是否为 UTF-8？
- [ ] **是否避免了使用 PowerShell 批量修改文件？**（绝对禁止，易产生中文乱码）
- [ ] 是否按照优先级顺序检查了库选择？
