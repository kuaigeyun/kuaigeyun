# RiverEdge SaaS 多租户框架 - 功能列表

## 📋 概述

本文档基于 [2.架构设计文档.md](./2.架构设计文档.md) 的架构设计，列出 **RiverEdge SaaS 多租户框架 (RiverEdge SaaS Multi-tenant Framework)** 的完整功能列表。
...
**核心组件**：
- 国际化配置（Umi 内置 @umijs/plugin-locale）
- 语言包管理（中文、英文等）
- 语言切换功能
- 日期时间本地化
- 数字格式化本地化
...
**功能说明**：超级管理员管理面板，用于监控租户运营情况、审核租户注册、管理系统配置

**核心组件**：
- 超管用户模型（SuperAdmin，独立于租户系统）
- 租户注册审核流程（待审核、已通过、已拒绝）
- 租户运营监控（租户数量、活跃度、资源使用情况）
- 租户数据统计（用户数、订单数、数据量等）
- 租户管理 API（审核、启用/禁用、删除）
- 系统级配置管理（全局配置、系统参数）
- 超管管理界面（租户列表、审核、监控看板）
...
- **租户运营监控**：
  - 租户数量统计（总数、活跃数、停用数）
  - 租户活跃度监控（日活、月活、访问量）
  - 租户资源使用监控（存储空间、API 调用次数、数据量）
  - 租户数据统计（用户数、订单数、业务数据量）
- **租户管理**：
  - 租户列表查询（支持筛选、排序、分页）
  - 租户详情查看
  - 租户启用/禁用
...
- [ ] 超管管理面板
  ...
  - [ ] 租户注册审核流程（待审核、已通过、已拒绝）
  - [ ] 租户注册管理 API（审核、启用/禁用、删除）
  - [ ] 租户运营监控 API（统计、活跃度、资源使用）
  - [ ] 租户数据统计 API（用户数、订单数、数据量）
...
本文档列出了 **RiverEdge SaaS 多租户框架 (RiverEdge SaaS Multi-tenant Framework)** 的完整功能列表：
