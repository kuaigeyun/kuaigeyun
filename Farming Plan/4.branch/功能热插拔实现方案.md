# RiverEdge SaaS 多租户框架 - 功能热插拔实现方案

## 📋 概述

在 **RiverEdge SaaS 多租户框架 (RiverEdge SaaS Multi-tenant Framework)** 中，功能热插拔是指在不修改核心代码的情况下，通过插件机制动态加载和卸载功能模块（如支付、短信、邮件等），实现功能的灵活扩展和替换。

## 🎯 设计目标

1. **核心代码零污染**：核心系统不包含任何业务功能的具体实现
2. **运行时热插拔**：支持在不重启服务的情况下启用/禁用功能
3. **接口标准化**：通过抽象接口定义功能契约，插件实现具体功能
4. **多实现支持**：同一功能可以有多个实现（如多种支付方式）
5. **配置驱动**：通过配置选择使用哪个实现
...
1. **接口隔离原则**：核心系统只定义接口，不包含实现
2. **依赖倒置原则**：核心系统依赖抽象接口，插件实现具体功能
3. **开闭原则**：对扩展开放，对修改关闭
4. **单一职责原则**：每个插件只实现一个功能
...
```python
# riveredge-seeds/seed-payment-alipay/backend/src/plugin.py
...
class AlipayPaymentPlugin(BasePlugin):
    ...
    def get_router(self) -> APIRouter:
        router = APIRouter(prefix="/api/payment/alipay", tags=["支付-支付宝"])
        # 注册插件特定的 API 路由（可选）
        return router
    
    def get_models(self) -> List:
        return []  # 支付插件可能不需要数据库模型
...
```python
# riveredge-seeds/seed-sms-aliyun/backend/src/plugin.py
from core.plugin_base import BasePlugin
...
class AliyunSMSPlugin(BasePlugin):
    ...
    def get_router(self) -> APIRouter:
        router = APIRouter(prefix="/api/sms/aliyun", tags=["短信-阿里云"])
        return router
...
class PaymentService:
    ...
    async def create_payment(
        ...
        if provider is None:
            raise ValueError(f"支付提供商不存在: {provider_name}")
...
class SMSService:
    ...
    async def send_sms(
        ...
        if provider is None:
            raise ValueError(f"短信提供商不存在: {provider_name}")
...
```
1. 系统启动时扫描 riveredge-seeds 目录
2. 读取插件的 manifest.json
3. 检查插件是否启用（enabled: true）
4. 动态导入插件模块
5. 实例化插件类
6. 调用插件的 on_load() 钩子
7. 插件在 on_load() 中注册服务提供商
8. 注册插件路由到 FastAPI
9. 注册插件模型到 Tortoise ORM
```

### 2. 插件卸载流程

```
1. 调用插件的 on_unload() 钩子
2. 插件在 on_unload() 中注销服务提供商
3. 从 FastAPI 中移除插件路由
4. 从 Tortoise ORM 中移除插件模型
5. 清理插件资源
```
...
1. **核心代码零污染**：核心系统只定义接口，不包含任何具体实现
2. **灵活扩展**：可以轻松添加新的支付方式、短信服务等
3. **运行时切换**：支持在不重启的情况下切换不同的实现
4. **多实现支持**：同一功能可以有多个实现，通过配置选择
5. **易于测试**：可以轻松模拟服务提供商进行单元测试
6. **版本管理**：每个插件独立版本管理，互不影响

## 🎯 最佳实践

### 1. 接口设计原则

- **接口要稳定**：接口定义后尽量不修改，避免影响现有插件
- **向后兼容**：接口变更时保持向后兼容
- **版本管理**：接口版本化，支持多版本共存
...
- **配置驱动**：通过配置文件管理插件行为
- **错误处理**：完善的错误处理和日志记录
- **资源清理**：在 on_unload() 中清理所有资源

### 3. 服务注册原则

- **命名规范**：服务提供商名称要唯一且有意义
- **默认提供**：每个功能至少有一个默认提供商
- **状态管理**：记录服务提供商的状态（启用/禁用）

## 📚 相关文档

- [插件开发规范.md](./插件开发规范.md) - 插件开发详细规范
- [插件目录结构说明.md](./插件目录结构说明.md) - 插件目录结构说明
- [2.架构设计文档.md](../1.plan/2.架构设计文档.md) - 系统架构设计
